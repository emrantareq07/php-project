...
date_default_timezone_set("Asia/Dhaka"); // GMT+6
session_name('blrr');
session_start();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($d_number) || empty($entry_date) || empty($subject)) {
        echo json_encode(['status' => 0, 'message' => 'Required fields are missing']);
        exit;
    }

    // ✅ Check if d_number already exists (excluding current record)
    $check_dnum = "SELECT id FROM `$table_name` WHERE d_number = '$d_number' AND id != '$id' LIMIT 1";
    $check_res = mysqli_query($conn, $check_dnum);
    if (mysqli_num_rows($check_res) > 0) {
        echo json_encode(['status' => 0, 'message' => 'Duplicate D Number! Update not allowed.']);
        exit;
    }
    ...
    // Insert into destination tables (if any)
            ...
            if ($dest_table) {
                ...
                if (mysqli_num_rows($check_table) == 1) {
                    $sql_insert = "INSERT INTO `$dest_table`
                        (`unique_id`, `recipient`, `immediate_sender_office`, `ref_number`, `send_date`, 
                         `sender`, `div_dept_office`, `subject`, `chairman_note`, `comments`, `medium`, `status`) 
                        VALUES 
                        ('$unique_id', '$dest', '$immediate_sender_office', '$ref_number', '$send_date', 
                         '$sender', '$div_dept_office', '$subject', '$chairman_note', '$comments', '$medium', 'pending')";
