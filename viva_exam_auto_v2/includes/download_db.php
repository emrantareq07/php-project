...
function createDatabaseBackup($outputFile){ 
    ...
    // Create DB dump
    $dump = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=viva_exam_db', 'root', '');
    $dump->start($outputFile);

    $f = date('d-m-Y');
    $dump1 = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=viva_exam_db', 'root', '');
    $dump1->start("../db/viva_exam_db$f.sql");
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = "BCIC Viva Exam Auto Database Backup " . $f;
        $mail->Body    = "Database Backup attached.";
        $mail->addAttachment("../db/viva_exam_db$f.sql");
...
if (isset($_POST['submit'])) {
    $backupFilename = '../db/viva_exam_db_' . date('d-m-Y') . '.sql';
