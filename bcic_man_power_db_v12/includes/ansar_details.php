...
<div class="container mt-3 shadow rounded p-4">
  <h2 class="text-muted text-center text-uppercase fw-bold">আনছার জনবল বিস্তারিত</h2>
  ...
  <table class="table table-bordered print-section">
    ...
    <tbody>
          ...
          <tr>
            ...
            <td class="no-print">
              <div class="btn-group">
                ...
                <!-- Edit -->
                <a href="ansar_info.php?id=<?php echo $row['id']; ?>&factory_name=<?php echo urlencode($factory_name); ?>" class="btn btn-warning btn-sm"
      ...
      <?php else: ?>
        <tr>
          <td colspan="9" class="text-center">No Ansar Record found</td>
        </tr>
      <?php endif; ?>
    </tbody>
    <tfoot>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Ansar Details</h5>
...
function generateCombineWorkersPrintView(monthsData) {
        ...
        <body>
            <div class="container-fluid">
                <div class="print-header">
                    ...
                    <h5 class="text-bangla">কারখানা/প্রতিষ্ঠান/প্রকল্পের নাম : সমন্বিত রিপোর্ট (আনছার)</h5>
    ...
    for (let idx = 0; idx < monthsData.length; idx++) {
        ...
        for (let i = 0; i < monthData.data.length; i++) {
            const record = monthData.data[i];
            const factoryName = record.factory_name || 'অনামী কারখানা';
            ...
            const maxLen = Math.max(designations.length, grades.length, sanctionedPosts.length, maleCounts.length, femaleCounts.length, totalCounts.length);
            
            for (let j = 0; j < maxLen; j++) {

                // const designation = designations[j] ? designations[j].trim() : '';
                // const grade = grades[j] ? grades[j].trim() : '';

                var designationRaw = designations[j] ? designations[j] : (j === 0 ? record.designation : '');
var gradeRaw = grades[j] ? grades[j] : (j === 0 ? record.grade : '');

var designation = cleanValue(designationRaw);
var grade = cleanValue(gradeRaw);

                const sanctioned = sanctionedPosts[j] ? parseInt(sanctionedPosts[j]) || 0 : 0;
                const male = maleCounts[j] ? parseInt(maleCounts[j]) || 0 : 0;
                const female = femaleCounts[j] ? parseInt(femaleCounts[j]) || 0 : 0;
                const total = totalCounts[j] ? parseInt(totalCounts[j]) || 0 : 0;
                const vacant = sanctioned - total;

                // Skip if no meaningful data
                if (sanctioned === 0 && male === 0 && female === 0 && total === 0 && designation === '' && grade === '') {
                    continue;
                }

                // Create unique key for grade-designation combination
                const key = `${grade}|${designation}`;
        ...
        const summaryArray = [];
        for (let key in gradeDesignationSummary) {
        ...
        summaryArray.sort(function(a, b) {
            const gradeA = parseInt(a.grade.replace('Grade ', '')) || 0;
            const gradeB = parseInt(b.grade.replace('Grade ', '')) || 0;
        ...
        printContent += `
                ...
                <div class="factory-details">
                    <h4 class="bengali-title">কারখানা ভিত্তিক বিস্তারিত বিবরণ (আনছার)</h4>
        `;

        // Display factory-wise details
        const factoryNames = [];
        for (let fname in factoryDetails) {
        ...
        for (let f = 0; f < factoryNames.length; f++) {
            const factoryName = factoryNames[f];
...
function generateSingleWorkerPrintView(data) {
    // Get all CSV data
    const rawDesignations = data.designation ? data.designation.split(',') : [];
    const rawGrades = data.grade ? data.grade.split(',') : [];
    ...
    for (let i = 0; i < maxLength; i++) {
        let designation = rawDesignations[i] ? rawDesignations[i].trim() : '';
        let grade = rawGrades[i] ? rawGrades[i].trim() : '';
        const sanctioned = sanctionedPosts[i] ? parseInt(sanctionedPosts[i]) || 0 : 0;
        const male = maleCounts[i] ? parseInt(maleCounts[i]) || 0 : 0;
        const female = femaleCounts[i] ? parseInt(femaleCounts[i]) || 0 : 0;
        const total = totalCounts[i] ? parseInt(totalCounts[i]) || 0 : 0;
        
        const hasValidGrade = grade !== '' && grade !== 'Grade ';
        const hasValidDesignation = designation !== '';
        const hasNonZeroData = sanctioned > 0 || male > 0 || female > 0 || total > 0;
        ...
        <body>
            <div class="container">
                <div class="print-header">
                    ...
                    <h5 class="bangla-text">বিদ্যমান জনবলের পরিসংখ্যান (আনছার)</h5>
    ...
    if (validEntries.length > 0) {
        ...
        for (let i = 0; i < validEntries.length; i++) {
            ...
            const vacant = entry.sanctioned - entry.total;
            ...
            const isPlaceholder = entry.designation === '';
            const designationClass = isPlaceholder ? 'placeholder-text' : '';
            //<td class="bangla-text ${designationClass}" style="text-align: left;">${entry.designation || '-'}</td>
        ...
        printContent += `
            <div class="alert alert-info text-center">
                <p class="bangla-text">এই রেকর্ডের জন্য কোন আনছার ডাটা পাওয়া যায়নি</p>
            </div>
        `;
    }
    ...
    printContent += `
        ...
        <div class="row mt-2">
           <div class="col-md-12 text-center">
                <div style="border-top: 0px solid #000; width: auto; margin: 10px 0 0 0; padding-top: 0px;">
                    <strong><small>সিস্টেম জেনারেটেড ডকুমেন্ট। স্বাক্ষরের প্রয়োজন নাই।</small></strong>
                </div>
            </div>
        </div>
...
function convertToBanglaFactory(factoryName) {
    ...
    const key = factoryName.toString().toLowerCase().trim();
