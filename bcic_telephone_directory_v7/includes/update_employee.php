...
if (!empty($image_path)) {
    ...
    $sql = "UPDATE emp_tbl SET 
        emp_id = ?, name = ?, designation = ?, office = ?, phone_office = ?, 
        intercom = ?, mobile = ?, email = ?, department = ?, division = ?, 
        image = ?, fax = ?, status = ?, updated_at = NOW() 
        WHERE id = ?";
    ...
    $stmt->bind_param(
        ...
        $mobile, $email, $department, $division, $image_path, $fax, $status, $id
    );
} else {
    ...
    $stmt->bind_param(
        "sssssssssssi",
        $emp_id, $name, $designation, $office, $phone_office, $intercom,
        $mobile, $email, $department, $division, $fax, $status, $id
    );
}
