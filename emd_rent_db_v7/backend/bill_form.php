...
if (isset($_POST['submit'])) {
    ...
    $payorder_date_gb = $_POST['payorder_date_gb'];
        ...
        $payorder_no_cb = $_POST['payorder_no_cb'];
    $payorder_date_cb = $_POST['payorder_date_cb'];
    ...
    $month_numeric_input = date('n', strtotime($month)); // Convert month name to a number (e.g., October -> 10)
    ...
    // if ($month_numeric_input < $month_numeric_db) {
       ...
       $check_query = "SELECT COUNT(*) as count FROM `$tenants_tbl` WHERE date LIKE '$formatted_date%'";
           //$check_query = "SELECT COUNT(*) as count FROM `$tenants_tbl` WHERE month = '$month' and year = '$year_of_date'";
        ...
        $sql = "INSERT INTO `$tenants_tbl` 
        (date, month, year, fiscal_year, tenants_name, actual_hr, actual_eb, actual_wb, actual_gb, actual_cb, 
        payorder_no_hr, payorder_date_hr, payorder_no_wb, payorder_date_wb, payorder_no_eb, payorder_date_eb, 
        payorder_no_gb, payorder_date_gb, payorder_no_cb, payorder_date_cb, payorder_no_comb, payorder_date_comb, 
        hr_in, eb_in, wasa_in, gb_in, cb_in, tax5, challan_no_tax, date_tax, vat15, challa_no_vat, date_vat, 
        hr_outs, eb_outs, wasa_outs, gb_outs, cb_outs,all_value) 
        VALUES 
        ('$date', '$month', '$year_of_date', '$fiscal_year', '$tenants_name', '$actual_hr', '$actual_eb', 
        '$actual_wb', '$actual_gb', '$actual_cb', '$payorder_no_hr', '$payorder_date_hr', '$payorder_no_wb', 
        '$payorder_date_wb', '$payorder_no_eb', '$payorder_date_eb', '$payorder_no_gb', '$payorder_date_gb', 
        '$payorder_no_cb', '$payorder_date_cb', '$payorder_no_comb', '$payorder_date_comb', '$hr_in', '$eb_in', 
        '$wasa_in', '$gb_in', '$cb_in', '$tax5', '$challan_no_tax', '$date_tax', '$vat15', '$challa_no_vat', 
        '$date_vat', '$hr_outs', '$eb_outs', '$wasa_outs', '$gb_outs', '$cb_outs','$all_value')";
...
<!--       <div class="card-header text-end"> 
        <h6 class="text-center text-secondary text-bold text-uppercase spicy-rice-regular">Tenants Name: <?php echo  $tenants_name; ?></h6>
      </div> -->
      <div class="card-body">
    ...
    $sql="SELECT * FROM `$tenants_tbl` ORDER BY id DESC LIMIT 1";           
      ...
      if(mysqli_num_rows($result)==0){
        ...
        $wasa_outs_db='0';
         $gb_outs_db='0';
         $cb_outs_db='0';
        $total_outs_db=$hr_outs_db + $eb_outs_db + $wasa_outs_db+$gb_outs_db+$cb_outs_db;
      }
      else{   
    ...
    ?>
          ...
          <table class="table">
            ...
            <tbody>
              ...
              <tr>          
            <th>House Bill</th>
            <th> <input type="text" class="form-control" id="actual_hr" onkeyup="calculateTotal_hr();calculateTotal_total(); calculateTotal_tax(); calculateTotal_vat();" onkeypress="return /[0-9]/i.test(event.key)" placeholder="Enter House Rent" name="actual_hr" ></th> 
           ...
           <th><input type="text" class="form-control" id="hr_in" onkeyup="calculateTotal_eb();calculateTotal_total();" onkeypress="return /[0-9]/i.test(event.key)" placeholder="Enter Electric Bill" name="hr_in" ></th>
            ...
            <th><input type="text" class="form-control" id="hr_outs"   value="<?php echo $hr_outs_db ?>"  name="hr_outs" ></th>
           ...
           <tr>
            <th>Water Bill</th>   
            <th> <input type="text" class="form-control" id="actual_wb" onkeyup="calculateTotal_hr();calculateTotal_total1();" onkeypress="return /[0-9]/i.test(event.key)" placeholder="Enter Water Bill" name="actual_wb" ></th> 
           ...
           <th><input type="text" class="form-control" id="wasa_in" onkeyup="calculateTotal_eb();calculateTotal_total1();" onkeypress="return /[0-9]/i.test(event.key)" placeholder="Enter Water Bill" name="wasa_in" ></th>
            <th><input type="text" class="form-control" id="wasa_outs"     name="wasa_outs" value="<?php echo $wasa_outs_db ?>" ></th>
           ...
           <tr>
            <th>Electric Bill</th>   
            <th> <input type="text" class="form-control" id="actual_eb" onkeyup="calculateTotal_hr();calculateTotal_total2();" onkeypress="return /[0-9]/i.test(event.key)" placeholder="Enter Electric Bill" name="actual_eb" ></th> 
           <th><input type="text" class="form-control" id="eb_in" onkeyup="calculateTotal_eb();calculateTotal_total2();" onkeypress="return /[0-9]/i.test(event.key)" placeholder="Enter Electric Bill" name="eb_in" ></th>
            <th><input type="text" class="form-control" id="eb_outs"   name="eb_outs" value="<?php echo $eb_outs_db ?>" ></th>
            ...
            <tr>
            <th>Gas Bill</th>   
            <th> <input type="text" class="form-control" id="actual_gb" onkeyup="calculateTotal_hr();calculateTotal_total3();" onkeypress="return /[0-9]/i.test(event.key)" placeholder="Enter Gas Bill" name="actual_gb" ></th> 
           <th><input type="text" class="form-control" id="gb_in" onkeyup="calculateTotal_eb();calculateTotal_total3();" onkeypress="return /[0-9]/i.test(event.key)" placeholder="Enter Gas Bill" name="gb_in" ></th>
            <th><input type="text" class="form-control" id="gb_outs"   name="gb_outs" value="<?php echo $gb_outs_db ?>" ></th>
        ...
        <tr>
            <th>Car Parking</th>   
            <th>
                <input type="text" class="form-control" id="actual_cb" onkeyup="calculateTotal_hr();calculateTotal_total4();" onkeypress="return /[0-9]/i.test(event.key)" placeholder="Enter Car Parking Bill" name="actual_cb" ></th> 
            <th>
                <input type="text" class="form-control" id="cb_in" onkeyup="calculateTotal_eb();calculateTotal_total4();" onkeypress="return /[0-9]/i.test(event.key)" placeholder="Enter Car Parking Bill" name="cb_in"> </th>
            <th>
                <input type="text" class="form-control" id="cb_outs" name="cb_outs" value="<?php echo $cb_outs_db ?>" >
            </th>
            <th>
                <input type="text" class="form-control" id="payorder_no_cb" value="" name="payorder_no_cb">
            </th>
            <th>
                <input type="date" class="form-control" id="payorder_date_cb" value="" name="payorder_date_cb">
            </th>
        </tr>
...
<!--             <div class="row">
    ...
    <div class="row">
        <div class="col-sm-12 mb-3">
            <textarea name="all_value" id="all_value"></textarea>
        </div>
    </div>
...
function calculateTotal_total4() {
    var actual_cb = parseFloat(document.getElementById('actual_cb').value) || 0;
    var cb_in = parseFloat(document.getElementById('cb_in').value) || 0;
    var cb_outs = parseFloat(<?php echo $cb_outs_db; ?>) || 0;
    var total = actual_cb - cb_in + cb_outs;
    document.getElementById('cb_outs').value = total.toFixed(2);
    calculateTotal_outs();    
}
  ...
  function calculateTotal_outs() {
    ...
    var cb_outs = parseFloat(document.getElementById('cb_outs').value) || 0;
    var total_outs = hr_outs + wasa_outs + eb_outs+gb_outs+cb_outs;
  ...
  function calculateTotal_hr() {
    ...
    const gb = parseFloat(document.getElementById('actual_gb').value) || 0;
     const cb = parseFloat(document.getElementById('actual_cb').value) || 0;    
    // Calculate the sum
    const total = hr + wb + eb+gb+cb;    
  ...
  function calculateTotal_eb() {
    ...
    const cb = parseFloat(document.getElementById('cb_in').value) || 0;    
    // Calculate the sum
    const total = hr + wb + eb+gb+cb;    
    // Display the total in the readonly input field
