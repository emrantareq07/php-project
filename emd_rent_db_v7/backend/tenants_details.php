...
$tenants_name = $_GET['tenants_name'] ?? '';
...
<div id="printableArea">  
    ...
    $result = mysqli_query($conn, "SELECT * FROM $tenants_tbl order by id desc" );

    $sql_option_match = "SELECT * FROM company WHERE table_name LIKE '%" . mysqli_real_escape_string($conn, $tenants_tbl) . "%'";
    $resultkkq = mysqli_query($conn, $sql_option_match);

    $rowoption_match = mysqli_fetch_assoc($resultkkq);
        //$tenants_name = $rowoption_match['address ']; 
    if ($result) {
    ...
    <i class="fa fa-newspaper-o receipt_for_tenants" data-toggle="tooltip"
       ...
       data-address="<?php echo $rowoption_match['address']; ?>"
       title="Receipt for Tenants" style="font-size:20px; color:yellowgreen;"></i>
      </a>&nbsp;

   <!-- tenants details slip    -->
<a href="#viewEmployeeModal_tenants_aa" class="edit" data-toggle="modal" style="text-decoration: none">
    <i class="fa fa-sticky-note receipt_for_tenants_aa" data-toggle="tooltip"
       data-tenants_name="<?php echo $row['tenants_name']; ?>"
       data-date="<?php echo $row['date']; ?>"
       data-all_value="<?php echo htmlspecialchars($row['all_value']); ?>"
       data-month="<?php echo $row['month']; ?>"
       data-actual_hr="<?php echo $row['actual_hr']; ?>"
       data-actual_wb="<?php echo $row['actual_wb']; ?>"
       data-actual_eb="<?php echo $row['actual_eb']; ?>" 
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <section class=" d-flex justify-content-center align-items-center">
              ...
              <img src="images/bcic_logo.png" alt="BCIC Logo" style="width:80px; height:auto; margin-bottom: 5px;">
              <br>
              <h5 style="margin-top:5px;margin-bottom: 3px;">BANGLADESH CHEMICAL INDUSTRIES CORPORATION</h5>
                  <h5 class="text-decoration-none righteous-regular" style="margin-bottom: 3px;">Estate Management Department (EMD)</h5>
                  <h6 class="text-decoration-none badge bg-primary" id="month_u" style="margin-bottom: 0px;"></h6>
              </div>
          </section>
          ...
          <section>
            <div class="text-left">
                <div style="margin-left:10px;">
                    <span id="" class="">TO</span> <br> 
                    <span id="">Director/MD/Manager/Proprietor</span><br>  
                    <span class="text-decoration-none" id="tenants_name_u"></span> <br>
                    <span id="address_u"></span> <br>                 
                </div>
            </div>
      </section>
<style>
...
<table class="table table-bordered table-fixed">
  <tbody> 
    ...
    <tr class="table-success">
      ...
      <th class="text-center">Amount/Bill (TK)</th>
    </tr>
    
    <tr>
      <td class="text-center">House Rent</td>
      <td class="text-center" id="actual_hr_u"></td>
    </tr>
    <tr> 
      <td class="text-center">Water Bill</td>
      <td class="text-center" id="actual_wb_u"></td>
    </tr>
    <tr> 
      <td class="text-center">Electricity Bill</td>
      <td class="text-center" id="actual_eb_u"></td>
    </tr>
    <tr> 
      <td class="text-center">Gas Bill</td>
      <td class="text-center" id="actual_gb_u"></td>
    </tr>
    <tr> 
      <td class="text-center">Car Parking</td>
      <td class="text-center" id="actual_cb_u"></td>
    </tr>
    <tr class="table-secondary">
      <th class="text-center">Total (TK):</th>   
      <th class="text-center" id="hr_total_u"></th>
    </tr>          
    <tr class="border-0">
      <th colspan="2" class="border-0" style="height: 10px;"></th> <!-- Spacer -->
    </tr>
    
    <tr class="table-secondary">
      <td class="text-center p-0 fw-bold">In word:</td>   
      <td class="text-center p-0 fw-bold" id="grand_total_u"></td>
    </tr> 

    <tr rowspan="2" class="border-0"><th colspan="2" class="border-0"></th></tr>   
    <tr class="border-0">
      <th colspan="2" class="border-0" style="height: 40px;"></th> 
    </tr>  

    <tr rowspan="2" class="border-0">
      <th class="border-0 border-top text-center">Prepared By </th>
      <th class="border-0 border-top text-center">
        ACA/DCA, EMD<br>For General Manager (EMD)
      </th>
    </tr>

    <tr class="border-0">
      <td colspan="2" class="border-0" style="height: 20px;">
        <b>CC TO:</b><br>1. Controller of Accounts, BCIC, Dhaka<br>2. Customer Copy<br>3. Office Copy
      </td> 
    </tr> 
  </tbody>
</table>
...
$(document).on('click', '.receipt_for_tenants', function() {
    ...
    .replace("TO", "TO<br>") // Add a line break after "TO"
    .replace("Director/MD/Proprietor", "Director/MD/Proprietor<br>");
    ...
    // Populate modal fields with the data
     $('#tenants_name_u').text( tenants_name);
  // $('#month_u').text('PaySlip of Month: ' + month);
   ...
   // $('#grand_total_u').html(grand_total + "&nbsp;&nbsp;(" + numberToWords(grand_total) + " TK Only)");
    ...
    $('#grand_total_u').html("" + numberToWords(hr_total) + " TK Only");
   ...
   // $('#grand_total_uu').text(numberToWords(grand_total) || 'Zero');
    ...
    // $('#address_u').text(address || '0');
$('#address_u').html(formattedAddress);
...
document.getElementById('print_ind_tenants').addEventListener('click', function () {
    ...
    labels.forEach(label => {
        ...
        const copyContent = `
            <div style="page-break-after: always; margin: 0; padding: 15px; border: 2px solid gray; border-radius: 10px;">
                <div style="text-align:right; margin-bottom:0;">
                    <h6 class="mb-0 badge bg-danger">${label}</h6>
                </div>
                ${originalContent}
                <div style="text-align: center; margin-top: 0; margin-bottom: 3px; font-size: 0.9rem;">
                <hr>
              ...
              * {
              ...
              @media print {
                  @page {
                      ...
                      margin: 15mm 5mm 5mm 5mm; /* Set custom margins if needed */
                  }
                  body {
                      margin: 0;
                      padding: 0;
                  ...
                  div {
                      page-break-inside: avoid; /* Avoid splitting divs across pages */
                  }
              }
          </style>
        </head>
...
<!-- View Modal Customaer Copy HTMLaaa -->
<div id="viewEmployeeModal_tenants_aa" class="modal fade">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="">                
            <div class="modal-header d-flex justify-content-center"> 
          ...
          <div class="modal-body" id="printableArea_ind_tenants_aa">
<section class=" d-flex justify-content-center align-items-center">
    <div class=" text-center">
    <img src="images/bcic_logo.png" alt="BCIC Logo" style="width:80px; height:auto; margin-bottom: 5px;">
    <br>
...
<section>
    <div class="text-left">
        <div style="margin-left:10px;">
            <span id="" class="">TO</span> <br> 
            <span id="">Director/MD/Manager/Proprietor</span><br> 
            <span class="text-decoration-none" id="tenants_name_uk"></span> <br>
            <span id="address_uk"></span> <br>                 
        </div>
    </div>
</section>
<style>
...
<table class="table table-bordered table-fixed">
 ...
 <div  id="all_value_uk" class="text-center"> </div>
<style>

#all_value_uk {
  width: auto;  /* Adjust width as needed */
  max-width: auto;  /* Prevent stretching too wide */
...
#all_value_uk table {
  ...
  border-collapse: collapse;  /* Ensures borders appear between cells */
  table-layout: fixed;  /* Forces table to take up the full space */
  margin: 0 auto;  /* Center the table horizontally */
}
/* Ensure the cells take full width and align well */
#all_value_uk td, #all_value_uk th {
  border: 1px solid gray;
  padding: 5px;
  text-align: center;
  word-wrap: break-word;
...
<div class="modal-footer">
    <input type="hidden" value="2" name="type">
    <input type="button" class="btn btn-outline-danger" data-bs-dismiss="modal" value="Cancel">
    <button type="button" class="btn btn-success" id="print_ind_tenants_aa"><i class="fa fa-print" style="font-size:16px"></i> Print</button>
    </div>
    </form>
</div>
</div>
...
///aaaaa
    // for receipt for tenants
$(document).on('click', '.receipt_for_tenants_aa', function() {
    var tenants_name = $(this).data("tenants_name");
    var month = $(this).data("month");
    var all_value = $(this).data("all_value");  // Retrieve the correct data
    // You can log all_data to check if it's correctly fetched 
    ...
    $('#month_uk').text('Bill: ' + month + ' ' + year);
    $('#tenants_name_uk').text(tenants_name);
 $('#all_value_uk').html(all_value); 

    $('#address_uk').html(formattedAddress);
    $('#viewEmployeeModal_tenants_aa').modal('show');
});

document.getElementById('print_ind_tenants_aa').addEventListener('click', function () {
    // Define the labels for each copy
    const labels = ["Customer Copy"];    
    // Store the original content of the printable area
    const originalContent = document.getElementById('printableArea_ind_tenants_aa').innerHTML;
    // Format date as '14 Nov 2024'
    const formattedDate = new Date().toLocaleDateString('en-GB', {
        day: 'numeric',
        month: 'short',
    ...
    labels.forEach(label => {
        // Insert the label into the original content, along with a footer
        const copyContent = `
            <div style="page-break-after: always; margin: 0; padding: 15px; border: 0px solid gray; border-radius: 10px;">
                <div style="text-align:right; margin-bottom:0;">
                    <h6 class="mb-0 badge bg-danger">${label}</h6>
                </div>
                ${originalContent}
                <div style="text-align: center; margin-top: 10; margin-bottom: 20px; font-size: 0.9rem;">
