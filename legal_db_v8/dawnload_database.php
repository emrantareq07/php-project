...
$dump = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=legal_db', 'root', '');
 $dump->start($outputFile);


$dump1 = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=legal_db', 'root', '');
$f=date('d-m-Y');
$dump1->start("db/legal_db_$f.sql");
...
$mail->IsHTML(true);
$mail->Subject="BCIC Legal Database Backup ".$f;
$mail->Body="Database Backup";
$mail->AddAttachment("db/legal_db_$f.sql");
...
if (isset($_POST['submit'])) {
    $backupFilename = 'db/legal_db_' . date('d-m-Y') . '.sql';
