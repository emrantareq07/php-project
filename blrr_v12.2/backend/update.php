...
date_default_timezone_set("Asia/Dhaka");   // Bangladesh time (GMT+6)
session_name('blrr');
session_start();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // ✅ Preserve old d_number if empty or zero
    $existing_d_number = '';
    $get_dnum = mysqli_query($conn, "SELECT d_number FROM $table_name WHERE id='$id'");
    if ($get_dnum && mysqli_num_rows($get_dnum) > 0) {
        $existing_row = mysqli_fetch_assoc($get_dnum);
        $existing_d_number = $existing_row['d_number'];
    }

    if (empty($d_number) || $d_number == '0') {
        $d_number = $existing_d_number;
    }
    ...
    // ✅ Insert into destination tables (if selected)
                ...
                if (mysqli_num_rows($check_table) == 1) {
                    $sql_insert = "INSERT INTO `$dest_table`
                        (`unique_id`, `recipient`, `immediate_sender_office`, `ref_number`, `send_date`, 
                         `sender`, `div_dept_office`, `subject`, `chairman_note`, `comments`, `medium`) 
                        VALUES 
