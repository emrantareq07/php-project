...
if(isset($_POST['delete_student']))
{
    ...
    if($query_run)
    {
        $_SESSION['message'] = "<span class='text-success'><b>User Information Deleted Successfully</b></span>";
...
if(isset($_POST['update_student']))
{
    ...
    $query = "UPDATE users SET  emp_id='$emp_id', designation='$designation', status='$status', role='$role' WHERE id='$id' ";
    $query_run = mysqli_query($conn, $query);
	 ...
	 else
    {
     ...
     $query = "UPDATE users SET  emp_id='$emp_id', designation='$designation', password='$hashedPassword', status='$status', role='$role' WHERE id='$id' ";
    $query_run = mysqli_query($conn, $query);
...
if(isset($_POST['save_adminuser'])){
    $emp_id = $_POST['username'];    
    $password = mysqli_real_escape_string($conn, $_POST['password']);
    $hashedPassword = md5($password);

    $query = "INSERT INTO users (emp_id,password,status,role,job_confirm_status) VALUES ('$emp_id','$hashedPassword','approved','admin','approved')";

    $query_run = mysqli_query($conn, $query);
    if($query_run){
        $_SESSION['message'] = "<span class='text-success'><b> Information Created Successfully</b></span>";
        header("Location: manage_user.php");
        exit(0);
    }
    else{
        $_SESSION['message'] = "<span class='text-danger'><b> Information Not Created</b></span>";
        header("Location: manage_user.php");
        exit(0);
    }
}
