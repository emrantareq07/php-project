...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save') {
    ...
    $despatch_date     = toNullIfEmpty($_POST['despatch_date']);
    $quantity          = toNullIfEmpty($_POST['quantity']);
    $pur_order_no      = toNullIfEmpty($_POST['pur_order_no']);
    ...
    if ($id > 0) {
        ...
        if ($stmt) {
            $stmt->bind_param(
                ...
                $port_arraival_date, $despatch_date, $quantity, $pur_order_no,
                $port_name, $status, $id
            );
            if ($stmt->execute()) {
        ...
        $sql = "INSERT INTO import_urea
                    (ref_no, shiping_date, country, ship_name, contractor_name,
                     port_arraival_date, despatch_date, quantity, pur_order_no,
                     port_name, status, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            $stmt->bind_param(
...
if ($search !== '') {
    $sql .= " AND (ref_no LIKE ? OR ship_name LIKE ? OR contractor_name LIKE ?
                   OR pur_order_no LIKE ? OR port_name LIKE ? OR country LIKE ?)";
    $like = "%$search%";
    for ($i = 0; $i < 6; $i++) {
        $params[] = $like;
    }
...
<style>
    * { box-sizing: border-box; }
    ...
    .btn-primary:hover { background: #2f65b0; }
    .btn-secondary { background: #e2e8f0; color: #333; }
    .btn-secondary:hover { background: #cbd5e0; }
    .btn-danger { background: #e53e3e; color: #fff; }
    .btn-danger:hover { background: #c53030; }
    .btn-edit { background: #38a169; color: #fff; }
    .btn-edit:hover { background: #2f855a; }
    ...
    .btn-allot { background: #805ad5; color: #fff; }
    .btn-allot:hover { background: #6b46c1; }
    .ref-context {
        display: flex;
        align-items: center;
        gap: 12px;
...
<a href="logout.php" class="btn btn-danger">Logout <i class="fa fa-sign-out"></i></a>
        ...
        <form method="POST" action="pur_dashboard.php">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="pur_order_no" value="<?php echo clean($conn, $editRow['pur_order_no'] ?? ''); ?>">
                </div>
                <div class="form-group">
                    <label>Country</label>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="port_name" value="<?php echo clean($conn, $editRow['port_name'] ?? ''); ?>">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="despatch_date" value="<?php echo clean($conn, $editRow['despatch_date'] ?? ''); ?>">
                </div>
                <div class="form-group">
    ...
    <div class="card">
        ...
        <form method="GET" action="pur_dashboard.php" class="search-bar">
            <input type="text" name="q" placeholder="Search by ref no, ship, contractor, PO no, port, country..." value="<?php echo clean($conn, $search); ?>">
        ...
        <table>
            ...
            <tbody>
                <?php if ($rows && $rows->num_rows > 0): ?>
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="15" style="text-align:center; padding:20px; color:#888;">No records found.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
        </div>
