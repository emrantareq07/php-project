...
// -----------------------------------------------------------------
// PORT LOCKED TO THE LOGGED-IN USER
// Each port login's username matches a port_name value in
// import_urea (e.g. a user named "chittagong_port" only ever sees
// Chittagong's data). If the username matches a real port_name,
// the port is locked to it and the picker/overview are skipped.
// If it doesn't match any port (e.g. an admin account), the normal
// full port picker + overview is shown as a fallback.
// -----------------------------------------------------------------
$isPortUser = false;
$chk = mysqli_prepare($conn, "SELECT COUNT(*) AS cnt FROM import_urea WHERE port_name = ?");
mysqli_stmt_bind_param($chk, 's', $logged_in_user);
...
// -----------------------------------------------------------------
$selectedPort = $isPortUser ? $logged_in_user : (isset($_GET['port']) ? trim($_GET['port']) : '');
...
// -----------------------------------------------------------------
// FETCH DISTINCT PORTS (from import_urea) — only used for the
// fallback full picker when the user isn't locked to one port
// -----------------------------------------------------------------
$portList = [];
if (!$isPortUser) {
...
// -----------------------------------------------------------------
// OVERVIEW: per-port summary (shown only for non-port-locked users
// when no port is selected yet)
// -----------------------------------------------------------------
$portOverview = [];
if (!$isPortUser && $selectedPort === '') {
...
<html lang="en">
<head>
  <title>BCIC SFMS — Port Dashboard</title>
  <meta charset="utf-8">
  ...
  <style>
    ...
    .welcome-banner h1 { font-size: 22px; margin: 0; }
    .welcome-banner .text-danger { color: #ffe08a !important; }

    .toolbar-actions .btn { border-radius: 6px; }
    ...
    .card-header {
        background: linear-gradient(135deg, #eef2ff, #e6f0ff) !important;
        border-bottom: 2px solid #e2e8f0;
        font-weight: 600;
        color: #1a3b5d;
    }
    .card.shipment-card { border-top: 4px solid var(--primary); }
    .card.allotment-card { border-top: 4px solid var(--accent); }
    ...
    .locked-port-badge {
        ...
        align-items: center;
        gap: 8px;
        box-shadow: 0 3px 8px rgba(128, 90, 213, 0.25);
    }
    ...
    .overview-card .card-title { color: #1a3b5d; }

    /* Summary pills */
    .summary-pill { padding: 5px 12px; border-radius: 14px; font-size: 13px; font-weight: 600; display: inline-block; margin-right: 6px; margin-bottom: 4px; }
    ...
    /* Status badge on shipment details */
    .status-badge {
        padding: 4px 11px;
        border-radius: 12px;
        font-size: 12px;
    ...
    .btn-secondary { background: #e2e8f0; color: #333; border: none; }
...
<!-- <nav class="navbar navbar-expand-lg navbar-dark">
  <div class="container-fluid text-center">
    <a class="navbar-brand" href="#">Digital Fertilizer Monitoring System (SFMS), BCIC.</a>
  </div>
</nav> -->
...
<div class="container-fluid page-shell p-1 my-1">
    <div class="row">
        <div class="col-sm-12">
            <div class="welcome-banner d-flex justify-content-between align-items-center flex-wrap">
                <h1>Welcome <b class="text-danger"><?= h($logged_in_user); ?></b> Dashboard</h1>
    ...
    <div class="row">
        <div class="col-sm-12"><h3 class="text-center text-uppercase mb-3">Port Dashboard</h3></div>
    </div>
        ...
        <div class="col-12">
            <form method="GET" action="" class="row g-2 align-items-end">
                <div class="col-md-4">
                    ...
                    <?php if ($isPortUser): ?>
                        <div>
                            <span class="locked-port-badge"><i class="fa fa-anchor"></i> <?= h($selectedPort); ?></span>
                            <input type="hidden" name="port" value="<?= h($selectedPort); ?>">
                        </div>
                    <?php else: ?>
                ...
                <div class="col-md-4">
                    <?php if ($isPortUser && $selectedRefNo !== ''): ?>
                        <a href="?port=<?= urlencode($selectedPort); ?>" class="btn btn-secondary"><i class="fa fa-times"></i> Clear Ref No</a>
                    <?php elseif (!$isPortUser && $selectedPort !== ''): ?>
    ...
    <?php if ($selectedPort === ''): ?>
        ...
        <!-- ============ OVERVIEW: ALL PORTS (fallback for non-port-locked users) ============ -->
        <div class="row">
            <div class="col-12">
            ...
            <div class="row mb-3">
                <div class="col-12">
                    <div class="card shipment-card">
                        <div class="card-header"><i class="fa fa-ship"></i> Shipment Details — Ref No <?= h($ureaRecord['ref_no']); ?></div>
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-3 mb-2">
                                    ...
                                    <span class="status-badge status-<?= h($ureaRecord['status']); ?>"><?= h(str_replace('_', ' ', $ureaRecord['status'])); ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-12">
                    <div class="card allotment-card">
                        <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
                            <span><i class="fa fa-list"></i> Allotments for Ref No <?= h($selectedRefNo); ?></span>
                            <div>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <button type="submit" name="send_transaction" value="1" class="btn btn-primary w-100" form="<?= $formId; ?>"><i class="fa fa-paper-plane"></i> Send</button>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php endif; ?>
