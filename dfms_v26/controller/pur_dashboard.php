...
// Must be called before any output/session access, and before any other
// file that might already have started the session gets a chance to.
...
// -----------------------------------------------------------------
// FETCH OFFICE LIST (for Buffer Name dropdown) — from `office_tbl`
// Assumed columns: id, office_name. Adjust column names below if
// your table uses different names.
// -----------------------------------------------------------------
$officeList = $conn->query("SELECT id, office_name FROM office_tbl ORDER BY office_name ASC");
$OFFICE_OPTIONS = [];
if ($officeList) {
    while ($off = $officeList->fetch_assoc()) {
        // buffer_name column stores the office name as text (not an FK id),
        // matching the existing schema for import_allotment.buffer_name
        $OFFICE_OPTIONS[] = $off['office_name'];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_allotment') {
    ...
    // Always taken from the server-side session, never trusted from the
    // posted (readonly) field, so it can't be tampered with or come back empty.
    $al_created_by = toNullIfEmpty($_SESSION['username'] ?? '');

    $validationError = '';

    if ($al_created_by === null) {
        $validationError = "Your session has expired or you are not logged in. Please log in again before adding an allotment.";
    }
            ...
            <tbody>
                <?php if ($rows && $rows->num_rows > 0): ?>
                    ...
                    <?php while ($r = $rows->fetch_assoc()): ?>
                        ...
                        <?php
                        // Get total allotted amount for this ref_no
                        $allottedAmount = 0;

                        $allotStmt = mysqli_prepare(
                            $conn,
                            "SELECT COALESCE(SUM(amount), 0) AS total_allotted
                             FROM import_allotment
                             WHERE ref_no = ?"
                        );

                        if ($allotStmt) {
                            mysqli_stmt_bind_param($allotStmt, 's', $r['ref_no']);
                            mysqli_stmt_execute($allotStmt);

                            $allotResult = mysqli_stmt_get_result($allotStmt);

                            if ($allotRow = mysqli_fetch_assoc($allotResult)) {
                                $allottedAmount = (float)$allotRow['total_allotted'];
                            }

                            // mysqli_stmt_close($allotStmt);
                        }
                        ...
                        // Remaining quantity
                        $remainingAmount = $quantity - $allottedAmount;

                        // Avoid very small floating-point negative values
                        if ($remainingAmount < 0 && $remainingAmount > -0.0001) {
                            $remainingAmount = 0;
                        }
                        ...
                        <tr>
                            ...
                            <td>
                                <?php echo clean($conn, $r['ref_no']); ?>
                            </td>

                            <td>
                                <?php echo clean($conn, $r['country']); ?>
                            </td>

                            <td>
                                <?php echo clean($conn, $r['ship_name']); ?>
                            </td>

                            <td>
                                <?php echo clean($conn, $r['contractor_name']); ?>
                            </td>

                            <td>
                                <?php echo clean(
                                    $conn,
                                    portLabel($PORT_OPTIONS, $r['port_name'])
                                ); ?>
                            </td>

                            <td>
                                <?php echo clean($conn, $r['shiping_date']); ?>
                            </td>

                            <td>
                                <?php echo clean($conn, $r['port_arraival_date']); ?>
                            </td>

                            <td>
                                <?php echo clean($conn, $r['quantity']); ?>
                            </td>
                            ...
                            <td>
                                ...
                                <?php else: ?>
                                    ...
                                    <span class="status-badge status-pending">Remaining : 
                            ...
                            <td class="actions-col">
                                ...
                                <a class="btn btn-edit"
                                   href="pur_dashboard.php?action=edit&id=<?php echo (int)$r['id']; ?>">
                                    Edit
                                </a>

                                <a class="btn btn-allot"
                                   href="pur_dashboard.php?ref_no=<?php echo urlencode($r['ref_no']); ?>#allotment">
                                    Allotment
                                </a>
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <select name="buffer_name">
                            ...
                            <?php
                            $currentBuffer = $editAllotment['buffer_name'] ?? '';
                            foreach ($OFFICE_OPTIONS as $officeName) {
                                $sel = ($currentBuffer === $officeName) ? 'selected' : '';
                                echo "<option value=\"" . clean($conn, $officeName) . "\" $sel>" . clean($conn, $officeName) . "</option>";
                            }
                            ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="created_by" value="<?php echo clean($conn, $_SESSION['username'] ?? ''); ?>" readonly>
                    </div>
                </div>
