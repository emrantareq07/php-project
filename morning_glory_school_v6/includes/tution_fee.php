...
if(isset($_SESSION["uid"]) || isset($_COOKIE['user_login'])) { 
    ...
    try {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            // Calculate totals for exam fees
...
$(document).ready(function() {
    ...
    $('#add_tution_fee').click(function() {
        var newRow = $('.tution-fee-row:first').clone();
    ...
    $('#add_exam_fee').click(function() {
        var newRow = $('.exam-fee-row:first').clone();
        newRow.find('select, input').val('');
        newRow.find('.duplicate-error').hide();
    ...
    function updateRemoveButtons(buttonClass) {
    ...
    $(document).on('click', '.remove-tution-row', function() {
        if ($('.tution-fee-row').length > 1) {
            ...
            checkDuplicateMonths('.tution-month-select', 'month');
            calculateTotals();
        }
        updateRemoveButtons('.remove-tution-row');
    });
    ...
    $(document).on('click', '.remove-exam-row', function() {
        if ($('.exam-fee-row').length > 1) {
            ...
            checkDuplicateMonths('.exam-term-select', 'term');
            calculateTotals();
        }
        updateRemoveButtons('.remove-exam-row');
    });

    // Check for duplicate months or terms
    function checkDuplicateMonths(selector, type) {
        let items = [];
    ...
    // Handle input changes for tuition fees
    $(document).on('change', '.tution-actual-input, .tution-payment-input', function() {
        const row = $(this).closest('.tution-fee-row');
        calculateTutionRowDue(row);
        calculateTotals();
    });

    // Handle input changes for exam fees
    $(document).on('change', '.exam-actual-input, .exam-payment-input', function() {
        const row = $(this).closest('.exam-fee-row');
        calculateExamRowDue(row);
        calculateTotals();
    });

    // Check for duplicate months when month selection changes
    $(document).on('change', '.tution-month-select', function() {
        checkDuplicateMonths('.tution-month-select', 'month');
    });

    // Check for duplicate terms when term selection changes
    ...
    $('#feeForm').on('submit', function(e) {
        if (checkDuplicateMonths('.tution-month-select', 'month') || 
            checkDuplicateMonths('.exam-term-select', 'term')) {
            e.preventDefault();
        ...
        let totalPay = 0;
        $('.tution-payment-input, .exam-payment-input').each(function() {
