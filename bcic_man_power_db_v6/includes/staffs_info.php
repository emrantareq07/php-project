                    ...
                    <thead class="table-light">
                        <tr>
                            <td colspan="8" class="text-end">
                                <!-- Add Row Button -->
                                <button type="button" id="add_staff" class="btn btn-success btn-sm" 
                    ...
                    <tbody id="staff_body">
                            ...
                            <tr class="staff-row">
                                ...
                                <td>
                                    <select class="form-select grade-select" name="grade[]" required>
                                        ...
                                        <?php for($j = 11; $j <= 20; $j++): ?>
                                        <option value="Grade <?php echo $j; ?>" 
                                            <?php echo ($grades[$i] ?? '') == "Grade $j" ? 'selected' : ''; ?>>
                                            Grade <?php echo $j; ?>
                                        </option>
                                        <?php endfor; ?>
                                    </select>
                                ...
                                <td class="text-center">
                                    <button type="button" class="btn btn-danger btn-sm remove-staff" 
                        ...
                        <?php else: ?>
                            ...
                            <tr class="staff-row">
                                ...
                                <td class="text-center">
                                    <button type="button" class="btn btn-danger btn-sm remove-staff" disabled>
                                        <i class="fa fa-trash"></i>
                                    </button>
                                </td>
                            </tr>
    ...
    <div class="card records-table">
        <div class="card-header bg-light">
            <div class="row align-items-center">
                <div class="col-md-6">
                    <h5 class="mb-0"><i class="fas fa-database"></i> Staff Records</h5>
                </div>
                <div class="col-md-6 text-end">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete this staff record? This action cannot be undone.</p>
...
$(document).ready(function() {
    let rowCount = $('#staff_body .staff-row').length;
...
// Check for duplicate grades when grade selection changes
$(document).on('change', '.grade-select', function() {
    checkDuplicate('.grade-select');
});

// Check for duplicates when form is submitted
function validateForm() {
    ...
    if (checkDuplicate('.grade-select')) {
        alert('Please fix duplicate grades before submitting.');
        return false;
    }
    ...
    if (!hasData) {
        alert('Please enter at least one staff record.');
        return false;
    }
...
// Also check for duplicates when new rows are added
...
// Also check for duplicates when rows are removed
$(document).on('click', '.remove-staff', function() {
    if (rowCount > 1) {
        $(this).closest('tr').remove();
        updateRowCount();
        // Check for duplicates after removing row
        checkDuplicate('.grade-select');
    } else {
    ...
    function updateRowCount() {
        rowCount = $('#staff_body .staff-row').length;
        
        // Enable/disable remove buttons
        if (rowCount > 1) {
            $('.remove-staff').prop('disabled', false);
        } else {
            $('.remove-staff:first').prop('disabled', true);
        }
    }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    window.location.href = 'staffs_info.php';
                } else {
    ...
    $('#load_save_btn').on('click', function() {
        ...
        const formData = $('#staffForm').serialize().replace(/record_id=[^&]*&?/, '');
    ...
    $('#update_btn').on('click', function() {
        ...
        console.log('Update Form Data:', $('#staffForm').serialize());
    ...
    function checkMonthlyRestriction() {
        ...
        $.ajax({
            url: 'check_monthly_entry_staff.php',
            type: 'POST',
            data: { 
                date: date,
    ...
    $(document).on('click', '.edit-btn', function() {
        ...
        window.location.href = 'staffs_info.php?edit_id=' + id;
    });

    // Load button behavior
    $(document).on('click', '.load-btn', function() {
        const id = $(this).data('id');
        window.location.href = 'staffs_info.php?load_id=' + id;
    });
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success) {
                ...
                <html>
                <head>
                    <title>Staff Data Report - ${data.date}</title>
                    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
                ...
                content += `
                    <div class="mt-4">
                        <h5 class="text-center">স্টাফদের তালিকা</h5>
                        <table class="print-table">
                            <thead>
                                <tr>
                                    <th>ক্রমিক</th>
                                    <th class="designation-cell">পদের নাম</th>
                                    <th>গ্রেড</th>
                                    <th>অনুমোদিত পদ</th>
                ...
                let detailedGrandMale = 0;
                let detailedGrandFemale = 0;
                let detailedGrandTotal = 0;
                let detailedGrandSanctioned = 0;
                let detailedGrandVacant = 0;
                    ...
                    if (designations[i] && designations[i].trim()) {
                        ...
                        content += `
                            <tr>
                                ...
                                <td class="designation-cell">${designations[i].trim()}</td>
                                <td>${grades[i] ? grades[i].trim() : ''}</td>
        ...
        error: function(xhr, status, error) {
            ...
            alert('Error loading record details. Please check if get_staff_data.php exists and check console for details.');
        }
    });
}
...
function showDetails(type, data) {
    ...
    switch(type) {     
        ...
        case 'total':
            ...
            `<div class="staff-details bg-primary text-white mt-2">
