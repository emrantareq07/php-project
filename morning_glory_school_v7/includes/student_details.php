    ...
    elseif (isset($_POST['update'])) {
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            if ($row['reg_no'] == $_POST['reg_no'] && $row['class'] == $_POST['class']) {
                $stmt2 = $conn->prepare("UPDATE $table_name SET roll_no=?, section=?, status=? WHERE reg_no=?");
                $stmt2->bind_param("ssss", $_POST['roll_no'], $_POST['section'], $_POST['status'], $_POST['reg_no']);
                $stmt2->execute();
                $stmt2->close();
            }
             else {
            ...
            <div class="card-body">
               <!-- Filter with Class and Section -->
                <div class="filter-section mb-0 d-flex justify-content-center">
                    <form method="get" action="student_details.php" class="row row-cols-lg-auto g-3 align-items-center">
                        ...
                        <div class="col-auto">
                            ...
                            <a href="student_details.php" class="btn btn-outline-danger">Reset Filters</a>
                        </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                <a href="sub_mark.php?reg_no=<?= urlencode($row['reg_no']) ?>" 
                                   class="btn btn-sm btn-info action-btn">
                                    <i class="fas fa-list"></i> Insert Mark
                                </a>
                            <?php else: ?>
                                <a href="javascript:void(0);" 
                                   class="btn btn-sm btn-secondary action-btn disabled" 
                                   style="pointer-events:none; opacity:0.6; cursor:not-allowed;">
                                    <i class="fas fa-list"></i> Insert Mark
                                </a>
                            <?php endif; ?>

                                <!-- <a href="sub_mark.php?reg_no=<?= $row['reg_no'] ?>" class="btn btn-sm btn-info action-btn">
                                    <i class="fas fa-list"></i> Insert Mark
                                </a> -->
                            <!-- <a href="sub_mark.php?reg_no=<?= urlencode($row['reg_no']) ?>&table_name=<?= urlencode($table_name) ?>" 
                           class="btn btn-sm btn-info action-btn">
                            <i class="fas fa-list"></i> Mark
                        </a> -->
