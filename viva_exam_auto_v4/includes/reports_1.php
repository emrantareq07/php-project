...
if (mysqli_num_rows($result) > 0) { 
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $committee = $row['committe_name'];

        // fetch ALL designations (repeated allowed)
        $q_des = "
            SELECT designation 
            FROM candidates_tbl
        ...
        ";
        $res_des = mysqli_query($conn, $q_des);

        // if no designation found
        if (mysqli_num_rows($res_des) == 0) { ?>
            <tr class="text-center">
                ...
                <td>
                    <a href="print_candidates.php?committee=<?= urlencode($committee) ?>" 
                       target="_blank" 
        ...
        <?php 
            ...
            // show each designation in separate row
            while ($d = mysqli_fetch_assoc($res_des)) { ?>
                <tr class="text-center">
                    <td><?= $i++ ?></td>
                    <td><?= htmlspecialchars($committee) ?></td>
    ...
    <tr>
        <td colspan="6" class="text-center text-muted">No committee schedules found.</td>
    </tr>
<?php } ?>
</tbody>
