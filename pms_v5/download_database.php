...
$dump = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=morning_glory_db', 'root', '');
 $dump->start($outputFile);

$dump1 = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=morning_glory_db', 'root', '');
$f=date('d-m-Y');
$dump1->start("db/morning_glory_db_$f.sql");
...
$mail->IsHTML(true);
$mail->Subject="Morning Glory Database Backup ".$f;
$mail->Body="Database Backup";
$mail->AddAttachment("db/morning_glory_db_$f.sql");
...
if (isset($_POST['submit'])) {
    $backupFilename = 'db/morning_glory_db_' . date('d-m-Y') . '.sql';
