...
// Handle Add
if (isset($_POST['add'])) {
    $stmt = $conn->prepare("INSERT INTO medicine_tbl (med_type, med_name, brand, quantity,dose) VALUES (?, ?, ?, ?, ?)");
    $stmt->execute([$_POST['med_type'], $_POST['med_name'], $_POST['brand'], $_POST['quantity'], $_POST['dose']]);
...
// Handle Edit
if (isset($_POST['edit'])) {
    $stmt = $conn->prepare("UPDATE medicine_tbl SET med_type=?, med_name=?, brand=?, quantity=?, dose=? WHERE id=?");
    $stmt->execute([$_POST['med_type'], $_POST['med_name'], $_POST['brand'], $_POST['quantity'],$_POST['dose'], $_POST['id']]);
    header("Location: medicine_mgtm.php");
    exit;
}
...
<div class="container py-4 border rounded ">
    ...
    <table id="medicineTable" class="table table-striped">
        ...
        <?php foreach ($medicines as $medicine): ?>
            <tr>
                <td><?= $medicine['id'] ?></td>
                <td><?= htmlspecialchars($medicine['med_type']) ?></td>
                ...
                <td>
                    <button class="btn btn-sm btn-warning editBtn" 
                            data-id="<?= $medicine['id'] ?>"
                            data-med_type="<?= htmlspecialchars($medicine['med_type']) ?>"
                            ...
                            data-quantity="<?= htmlspecialchars($medicine['dose']) ?>"
    ...
    <div class="modal-dialog">
        <form method="post" class="modal-content">
            ...
            <div class="modal-body">
                <input name="med_type" class="form-control mb-2" placeholder="Medicine Type" required>
                ...
                <input name="dose" class="form-control mb-2" placeholder="Dose" >
            </div>
    ...
    <div class="modal-dialog">
        <form method="post" class="modal-content">
            ...
            <div class="modal-body">
                ...
                <input name="med_type" id="editType" class="form-control mb-2" placeholder="Medicine Type" required>
                ...
                <input name="quantity" id="editQuantity" type="number" class="form-control mb-2" placeholder="Quantity" >
                <input name="dose" class="form-control mb-2" id="editDose" placeholder="Dose" >
            </div>
            <div class="modal-footer">
...
$(document).ready(function() {
    ...
    $('.editBtn').click(function() {
        ...
        $('#editType').val($(this).data('med_type'));
        $('#editName').val($(this).data('med_name'));
        $('#editBrand').val($(this).data('brand'));
        $('#editQuantity').val($(this).data('quantity'));
        $('#editDose').val($(this).data('dose'));
    });
