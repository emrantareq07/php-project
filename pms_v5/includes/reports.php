...
<head>
    ...
    <style>
        ...
        /* STRICT PRINT OPTIMIZATION RULES */
        @media print {
            /* Hide UI controls */
            .no-print { 
            ...
            /* REMOVE ALL CONTAINER & CARD BORDERS */
            .report-card, .card, .card-body, .border, .rounded { 
            ...
            /* FORCE HIGH-CONTRAST SOLID BLACK TABLE BORDERS */
            ...
            /* Maintain pure text color for print */
            .text-primary, .text-success, .text-warning, .text-secondary { 
                color: #000000 !important; 
            }

            /* Badges & metric bars formatting */
            .badge {
                border: 1px solid #000000 !important;
                color: #000000 !important;
                background: transparent !important;
    ...
    <div class="report-card p-4">
        ...
        <div class="row align-items-center mb-3">
            ...
            <div class="col-8 text-center">
                ...
                <p class="text-muted mb-1 small" style="font-size: 0.78rem;">
        ...
        <!-- Summary Metrics Bar -->
        <div class="d-flex justify-content-between align-items-center mb-3 p-2 bg-light metric-bar">
            <div class="d-flex gap-2">
                <span class="badge bg-primary metric-badge">
                    Prescriptions: <strong><?= $total_prescriptions ?></strong>
                </span>
                <span class="badge bg-success metric-badge">
                    Inventory Meds Issued: <strong><?= $total_reg_qty ?></strong>
                </span>
                <span class="badge bg-warning text-dark metric-badge">
                    Custom Meds Issued: <strong><?= $total_custom_qty ?></strong>
                </span>
            </div>
            <span class="text-muted small">Generated: <?= date('d-M-Y h:i A') ?></span>
        </div>

        <!-- Employee Info Section (Only when searching by Employee ID) -->
        <?php if (!empty($emp_id)): ?>
            <div class="mb-3">
                <strong class="text-primary d-block mb-1 small" style="font-size: 0.8rem;">
                    ...
                    <?php if (!empty($registered_report)): ?>
                        <?php $i = 1; foreach ($registered_report as $item): ?>
                            <tr>
                                ...
                                <td class="text-center"><span class=""><?= htmlspecialchars($item['unit']) ?></span></td>
