...
// Handle Add
if (isset($_POST['add'])) {
    $stmt = $conn->prepare("INSERT INTO medicine_tbl (med_type, med_name, brand, quantity) VALUES (?, ?, ?, ?)");
    $stmt->execute([$_POST['med_type'], $_POST['med_name'], $_POST['brand'], $_POST['quantity']]);
...
// Handle Edit
if (isset($_POST['edit'])) {
    $stmt = $conn->prepare("UPDATE medicine_tbl SET med_type=?, med_name=?, brand=?, quantity=? WHERE id=?");
    $stmt->execute([$_POST['med_type'], $_POST['med_name'], $_POST['brand'], $_POST['quantity'], $_POST['id']]);
    header("Location: medicine_mgtm.php");
    exit;
}
