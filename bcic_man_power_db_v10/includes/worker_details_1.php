...
<div class="container mt-3 shadow rounded p-4">
  <h2 class="text-muted text-center text-uppercase fw-bold">All Workers From All Factory</h2>
...
// Function to generate combine workers print view with grade and designation breakdown in Bangla
function generateCombineWorkersPrintView(monthsData) {
        ...
        <head>
            ...
            <style>
                ...
                @media print {
                    ...
                    .designation-table { font-size: 11px; }
                }
                .text-bangla {
                    font-family: 'Noto Sans Bengali', 'SolaimanLipi', Arial;
                }
        ...
        <body>
            <div class="container-fluid">
                <div class="print-header">
                    ...
                    <h4 class="text-bangla">কারখানা/প্রতিষ্ঠান/প্রকল্পের নাম : সমন্বিত রিপোর্ট (শ্রমিক)</h4>
    ...
    monthsData.forEach(monthData => {
        ...
        // Initialize all grades for workers
        const allGrades = ['Grade 1', 'Grade 2', 'Grade 3', 'Grade 4', 'Grade 5', 'Grade 6', 'Grade 7', 'Grade 8', 'Grade 9', 'Grade 10', 'Grade 11', 'Grade 12', 'Grade 13', 'Grade 14', 'Grade 15', 'Grade 16'];
        allGrades.forEach(grade => {
            gradeDesignationSummary[grade] = {};
        });
        ...
        monthData.data.forEach(record => {
            ...
            designations.forEach((designation, index) => {
                ...
                // Add to grade designation summary
                if (grade && gradeDesignationSummary[grade]) {
                    if (!gradeDesignationSummary[grade][designationName]) {
                        gradeDesignationSummary[grade][designationName] = {
                            sanctioned: 0,
                            male: 0,
                            female: 0,
                            total: 0,
                    ...
                    gradeDesignationSummary[grade][designationName].sanctioned += sanctioned;
                    gradeDesignationSummary[grade][designationName].male += male;
                    gradeDesignationSummary[grade][designationName].female += female;
                    gradeDesignationSummary[grade][designationName].total += total;
                    gradeDesignationSummary[grade][designationName].vacant += vacant;
                }
        ...
        // Add grade-wise summary with designation breakdown
        let hasData = false;
        allGrades.forEach(grade => {
            const gradeData = gradeDesignationSummary[grade];
            const designations = Object.keys(gradeData);
            
            if (designations.length > 0) {
                hasData = true;
                let gradeSanctionedTotal = 0;
                let gradeMaleTotal = 0;
                let gradeFemaleTotal = 0;
                let gradeTotal = 0;
                let gradeVacantTotal = 0;

                printContent += `
                    <div class="grade-section">
                        <h5 class="grade-header bangla-text">
                            ${convertGradeToBangla(grade)} - পদভিত্তিক বিবরণ
                        </h5>
                            ...
                            <table class="designation-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th width="35%" class="bangla-text">পদের নাম</th>
                                        ...
                                        <th width="12%" class="bangla-text">খালি পদ</th>
                                    </tr>
                                </thead>
                                <tbody>
                `;

                // Add designation rows for this grade
                let designationSerial = 1;
                designations.forEach(designation => {
                    const desigData = gradeDesignationSummary[grade][designation];
                    gradeSanctionedTotal += desigData.sanctioned;
                    gradeMaleTotal += desigData.male;
                    gradeFemaleTotal += desigData.female;
                    gradeTotal += desigData.total;
                    gradeVacantTotal += desigData.vacant;

                    printContent += `
                        <tr>
                            <td class="bangla-number">${englishToBanglaNumber(designationSerial)}</td>
                            <td class="bangla-text" style="text-align: left;">${designation}</td>
                            <td class="bangla-number">${englishToBanglaNumber(desigData.sanctioned)}</td>
                            <td class="bangla-number">${englishToBanglaNumber(desigData.male)}</td>
                            <td class="bangla-number">${englishToBanglaNumber(desigData.female)}</td>
                            <td class="bangla-number">${englishToBanglaNumber(desigData.total)}</td>
                            <td class="bangla-number">${englishToBanglaNumber(desigData.vacant)}</td>
                        </tr>
                    `;
                    designationSerial++;
                });
                ...
                // Add grade total row
                printContent += `
                                <tr class="designation-total">
                                    <td colspan="2" class="bangla-text" style="text-align: right;"><strong>${convertGradeToBangla(grade)} মোট:</strong></td>
                                    <td class="bangla-number"><strong>${englishToBanglaNumber(gradeSanctionedTotal)}</strong></td>
                                    <td class="bangla-number"><strong>${englishToBanglaNumber(gradeMaleTotal)}</strong></td>
                                    <td class="bangla-number"><strong>${englishToBanglaNumber(gradeFemaleTotal)}</strong></td>
                                    <td class="bangla-number"><strong>${englishToBanglaNumber(gradeTotal)}</strong></td>
                                    <td class="bangla-number"><strong>${englishToBanglaNumber(gradeVacantTotal)}</strong></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
        ...
        if (!hasData) {
            printContent += `
                <div class="alert alert-info text-center">
                    <p class="bangla-text">এই মাসের জন্য কোন শ্রমিক ডাটা পাওয়া যায়নি</p>
                </div>
            `;
        }
        ...
        printContent += `
                <div style="margin-top: 30px;">
                    <h5 class="bengali-title">সর্বমোট সারসংক্ষেপ</h5>
                    <table class="summary-table">
                        <thead>
                            <tr>
                                <th class="bangla-text" width="8%">ক্রমিক</th>
                                <th class="bangla-text" width="25%">গ্রেড</th>
                                <th class="bangla-text" width="15%">অনুমোদিত পদ</th>
                                <th class="bangla-text" width="12%">পুরুষ</th>
                                <th class="bangla-text" width="12%">মহিলা</th>
                                <th class="bangla-text" width="12%">মোট</th>
                                <th class="bangla-text" width="15%">খালি পদ</th>
                            </tr>
                        </thead>
                        <tbody>
        `;

        // Calculate grade-wise totals for summary
        const gradeSummary = {};
        allGrades.forEach(grade => {
            gradeSummary[grade] = { sanctioned: 0, male: 0, female: 0, total: 0, vacant: 0 };
        });

        // Process all data to calculate grade totals
        monthData.data.forEach(record => {
            const designations = record.designation ? record.designation.split(',') : [];
            const grades = record.grade ? record.grade.split(',') : [];
            const sanctionedPosts = record.sanctioned_post ? record.sanctioned_post.split(',') : [];
        ...
        printContent += `
                        <tr class="total-row">
                            <td colspan="2" class="bangla-text"><strong>সর্বমোট</strong></td>
        ...
        // Add factory-wise details section (optional - you can remove this if not needed)
        printContent += `
                <div class="factory-details" style="margin-top: 30px;">
                    <h5 class="bengali-title">কারখানা ভিত্তিক বিস্তারিত (শ্রমিক)</h5>
        `;
                ...
                <div style="margin-bottom: 15px;">
                    <table class="designation-table">
                        <thead>
                            ...
                            <tr>
                                <th width="5%" class="bangla-text">ক্রমিক</th>
                                <th width="25%" class="bangla-text">পদের নাম</th>
