...
$dump = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=bcic_library_db', 'root', '');
$dump->start($outputFile);

$dump1 = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=bcic_library_db', 'root', '');
$f=date('d-m-Y');
$dump1->start("db/bcic_library_db$f.sql");
...
$mail->IsHTML(true);
$mail->Subject="BCIC e-Library Database Backup ".$f;
$mail->Body="Database Backup";
$mail->AddAttachment("db/bcic_library_db$f.sql");
...
if (isset($_POST['submit'])) {
    $backupFilename = 'db/bcic_library_db_' . date('d-m-Y') . '.sql';
