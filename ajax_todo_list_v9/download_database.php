...
$dump = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=todolist_db', 'root', '');
 $dump->start($outputFile);

$dump1 = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=todolist_db', 'root', '');
$f=date('d-m-Y');
$dump1->start("db/todolist_db_$f.sql");
...
$mail->IsHTML(true);
$mail->Subject="Daily Meeting Schedule Database Backup ".$f;
$mail->Body="Database Backup";
$mail->AddAttachment("db/todolist_db_$f.sql");
...
if (isset($_POST['submit'])) {
    $backupFilename = 'db/todolist_db_' . date('d-m-Y') . '.sql';
