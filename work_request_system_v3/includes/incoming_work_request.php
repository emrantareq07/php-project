...
$status_filter = $_GET['status'] ?? 'all'; // Changed from 'incomplete' to 'all'
...
$params = [];
$types = '';
$base_conditions = ''; // Store base conditions for counting

// Check if user is IT Section Head
$isITSectionHead = ($user_db_division === 'IT Division' && $routine_role === 'section_head');
...
if ($routine_role === 'section_head') {
    ...
    // For IT Section Heads, show only ICT incomplete requests
    if ($isITSectionHead) {
        $sql_query .= " AND LOWER(w_com_section) = LOWER(?) AND w_req_type = 'ICT' AND w_com_status = 'incomplete'";
        $base_conditions .= " AND LOWER(w_com_section) = LOWER('" . $conn->real_escape_string($user_db_section) . "') 
                              AND w_req_type = 'ICT' 
                              AND w_com_status = 'incomplete'";
        $params[] = $user_db_section;
        $types .= 's';
        $req_type_filter = 'ICT'; // Force ICT filter
        $status_filter = 'incomplete'; // Force incomplete filter
    } else {
        // Regular section head logic
    ...
    $base_conditions .= " AND LOWER(w_com_division) = LOWER('" . $conn->real_escape_string($user_db_division) . "')";
...
if ($access_granted) {
    ...
    if ($routine_role === 'section_head') {
        if ($isITSectionHead) {
            $type_stats_sql .= " AND LOWER(w_com_section) = LOWER('" . $conn->real_escape_string($user_db_section) . "') 
                                 AND w_req_type = 'ICT' 
                                 AND w_com_status = 'incomplete'";
        } else {
            $type_stats_sql .= " AND LOWER(w_com_section) = LOWER('" . $conn->real_escape_string($user_db_section) . "')";
        ...
        $type_stats_sql .= " AND LOWER(w_com_division) = LOWER('" . $conn->real_escape_string($user_db_division) . "')";
    }
                    ...
                    <div class="col-12">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <p class="text-muted mb-0">Track and manage incoming work requests</p>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <div class="fw-bold">
                                    <?php 
                                    if ($isITSectionHead) {
                                        echo '<span class="badge bg-info">ICT Incomplete Requests Only</span>';
                ...
                <div class="alert alert-info mb-4">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <p class="mb-0">
                                <strong>You are viewing:</strong> Only <span class="badge bg-primary">ICT</span> type requests 
                                that are <span class="badge bg-warning">Incomplete</span>
                                <br>
