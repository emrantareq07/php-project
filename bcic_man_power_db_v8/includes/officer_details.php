  ...
  <table class="table table-bordered print-section">
    ...
    <tbody>
      <?php if ($result && $result->num_rows > 0): ?>
        <?php 
        // Group records by month-year to show combine print only once per month
        $monthGroups = [];
        ...
        while ($row = $result->fetch_assoc()): 
          ...
          // Check if this is the first record of the month
          $isFirstRecord = $monthGroups[$month_key][0]['id'] === $row['id'];
        ?>
          <tr>
            ...
            <td class="no-print">
              <div class="btn-group">
               ...
               <!-- Edit -->
               <a href="officers_info.php?id=<?php echo $row['id']; ?>&factory_name=<?php echo urlencode($row['factory_name']); ?>" class="btn btn-warning btn-sm"
            ...
            <td class="no-print">
                <?php if ($isFirstRecord): ?>
                    <!-- Combine Print Button (only show on first record of the month) -->
                    <button class="btn btn-success btn-sm print-btn" 
                            data-id="<?php echo $row['id']; ?>" 
                            title="Print Report for <?php echo $month_year; ?>">
                      <i class="fas fa-print"></i> Print
                    </button>
                <?php else: ?>
                    <!-- Show indicator for other records in the same month -->
                    <span class="badge bg-secondary">Use Print from first record</span>
                <?php endif; ?>
            </td>
          </tr>
        <?php endwhile; ?>
