...
// Officer gender sum (numeric columns)
function getOfficerGenderSum($conn, $table, $prefixList, $username, $yearMonth){
    $sumMale = $sumFemale = 0;
    foreach($prefixList as $g){
        $sql = "SELECT COALESCE(SUM({$g}_m),0) AS male, COALESCE(SUM({$g}_f),0) AS female FROM $table WHERE factory_name=? AND date LIKE CONCAT(?, '%')";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ss",$username, $yearMonth);
        $stmt->execute();
