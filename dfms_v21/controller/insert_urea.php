...
if(mysqli_num_rows($result) > 0){
  ...
  header("location:dashboard.php?already_exists=already_exists");
  exit(); 
} 
//main else
else { 
...
$now = new DateTime($date); //inputed date
	...
	if($now < $date22) {
	    header("location:dashboard.php?past_dated=Back date error");
	    exit(); // Stop further execution
	}
	elseif($now > $current_date2) {
	    header("location:dashboard.php?advance_dated=Advanced date error");
	    exit(); // Stop further execution
	}
