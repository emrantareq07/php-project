...
$dump = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=dfms_db', 'root', '');
 $dump->start($outputFile);


$dump1 = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=dfms_db', 'root', '');
$f=date('d-m-Y');
$dump1->start("../db/dfms_db_$f.sql");
...
$mail->IsHTML(true);
$mail->Subject="BCIC DFMS Database Backup ".$f;
$mail->Body="Database Backup";
$mail->AddAttachment("../db/dfms_db_$f.sql");
...
if (isset($_POST['submit'])) {
    $backupFilename = '../db/dfms_db_' . date('d-m-Y') . '.sql';
