...
// Detect device type for default view
...
$default_view = 'table'; // Default for desktop
if ($is_mobile && !$is_tablet) {
    $default_view = 'cards'; // Cards for mobile phones
} elseif ($is_tablet) {
    $default_view = 'cards'; // Cards for tablets (you can change to 'table' if preferred)
}

// Check if view preference was submitted
if (isset($_POST['view_preference'])) {
    $_SESSION['preferred_view'] = $_POST['view_preference'];
    $default_view = $_POST['view_preference'];
}
// Check if user has manually selected a view
elseif (isset($_SESSION['preferred_view'])) {
    $default_view = $_SESSION['preferred_view'];
}
?>  
...
<div class="row my-3 align-items-center">
    ...
    <div class="col-12 col-md-3 mb-2 mb-md-0">
        <form class="row g-2 align-items-center" action="" method="post" id="search-form">
            <input type="hidden" name="view_preference" id="view_preference" value="<?php echo $default_view; ?>">
            <div class="col-8 col-sm-8">
                <input type="date" class="form-control" name="date" id="date" required value="<?php echo isset($_POST['date']) ? $_POST['date'] : $yesterday; ?>">
            </div>
    ...
    <div class="col-12 col-md-6 text-center">
        ...
        <div class="mt-2">
            <small class="badge 
                <?php 
                if ($is_mobile && !$is_tablet) echo 'bg-info';
                elseif ($is_tablet) echo 'bg-warning text-dark';
                else echo 'bg-secondary';
                ?>" id="mobile_desktop_view">
                <?php 
                if ($is_mobile && !$is_tablet) echo '📱 Mobile View';
                elseif ($is_tablet) echo '📟 Tablet View';
                else echo '💻 Desktop View';
                ?>
            </small>
        </div>
    </div>
...
@media print {
    ...
    .view-toggle, #reload-btn, #login-btn, #print-btn, #search-btn, #mobile_desktop_view {
...
function toggleView(viewType, saveToStorage = true) {
    if (viewType === 'cards') {
        ...
        // Update hidden field
        document.getElementById('view_preference').value = 'cards';
        
        if (saveToStorage) {
            // Save to localStorage
            localStorage.setItem('bcic_preferred_view', 'cards');
        }
    } else {
        ...
        // Update hidden field
        document.getElementById('view_preference').value = 'table';
        
        if (saveToStorage) {
            // Save to localStorage
            localStorage.setItem('bcic_preferred_view', 'table');
        }
    }
}
...
function detectScreenSize() {
    ...
    if (width < 768) { // Mobile
        ...
        return 'cards'; // Change to 'table' if you prefer table view on tablets
    } else { // Desktop
        return 'table';
    }
}

// Initialize view based on device and user preference
function initializeView() {
    // Get PHP default view
    const phpDefaultView = '<?php echo $default_view; ?>';
    
    // Check localStorage for saved preference
    const savedView = localStorage.getItem('bcic_preferred_view');
    ...
    if (savedView) {
        ...
        viewToUse = savedView;
    } else if (phpDefaultView) {
        // Use PHP default view (from form submission or session)
        viewToUse = phpDefaultView;
    } else {
        // Use device detection
        viewToUse = detectScreenSize();
    }
    
    // Apply the view (don't save to localStorage on initial load)
    toggleView(viewToUse, false);
    
    // Make sure hidden field is updated
    document.getElementById('view_preference').value = viewToUse;
}

// Update view on window resize (optional)
let resizeTimer;
window.addEventListener('resize', function() {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(function() {
        // Only auto-switch if user hasn't set a manual preference
        if (!localStorage.getItem('bcic_preferred_view')) {
            const newView = detectScreenSize();
            toggleView(newView);
        }
    }, 250); // Delay to avoid too many triggers
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (searchForm) {
        searchForm.addEventListener('submit', function(e) {
            ...
            const isCardView = document.getElementById('card-view').style.display === 'block';
            const currentView = isCardView ? 'cards' : 'table';
            
            // Update hidden field with current view
            document.getElementById('view_preference').value = currentView;
...
// Update device badge on client side
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (badge) {
        ...
        badge.classList.remove('bg-info', 'bg-warning', 'bg-secondary', 'text-dark');
        
        // Add new class based on device
        switch(deviceType) {
            case 'mobile':
                badge.classList.add('bg-info');
                badge.innerHTML = '📱 Mobile View (Auto-detected)';
                break;
            case 'tablet':
                badge.classList.add('bg-warning', 'text-dark');
                badge.innerHTML = '📟 Tablet View (Auto-detected)';
                break;
            default:
                badge.classList.add('bg-secondary');
                badge.innerHTML = '💻 Desktop View (Auto-detected)';
        }
    }
});
</script>
...
<div id="printableArea_ind_tenants_aa" class="mb-0 m-0 p-0">
    ...
    <div class="view-toggle">
        <button id="table-btn" class="view-toggle-btn <?php echo $default_view == 'table' ? 'active' : ''; ?>" onclick="toggleView('table')">
            <i class="fa fa-table"></i> Table View
        </button>
        <button id="card-btn" class="view-toggle-btn <?php echo $default_view == 'cards' ? 'active' : ''; ?>" onclick="toggleView('cards')">
            <i class="fa fa-th-large"></i> Card View
        </button>
    </div>
    
    <!-- Table View -->
    <div id="table-view" style="<?php echo $default_view == 'table' ? 'display: block;' : 'display: none;'; ?>">
        <!-- Your existing table view code remains exactly the same -->
        <div class="row">
            <div class="col-sm-12">
    ...
    <div id="card-view" style="<?php echo $default_view == 'cards' ? 'display: block;' : 'display: none;'; ?>">
        <div class="row">
            <?php
                ...
                <div class="col-12 col-md-6 col-lg-4 mb-4">
                    <div class="card factory-card">
                        <div class="card-header factory-header">
                            <h5 class="factory-title text-uppercase"><?php echo $cardIndex . '. ' . htmlspecialchars($factory_name, ENT_QUOTES, 'UTF-8'); ?></h5>
                        </div>
            ...
            if (empty($allFactories)) {
               echo '<div class="col-12 text-center py-5 border border-danger rounded p-5">';
