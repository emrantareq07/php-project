...
if(isset($_SESSION["uid"]) || isset($_COOKIE['user_login'])) { 
  ...
  if (isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'delete') {
      ...
      $check = mysqli_query($conn, "SELECT id FROM innovation_tbl WHERE inventors_designation = (SELECT designation_bn FROM designation WHERE id = '$id')");
      if (mysqli_num_rows($check) > 0) {
            ...
            <div class="table-responsive">
                <table id="designationTable" class="table table-hover" style="width:100%">
                    ...
                    <tbody>
                        <?php
                        $query = "SELECT d.*, 
                                 (SELECT COUNT(*) FROM innovation_tbl WHERE inventors_designation = d.designation_bn) as usage_count 
                                 FROM designation d 
                                 ORDER BY d.id DESC";
                        $result = mysqli_query($conn, $query);
