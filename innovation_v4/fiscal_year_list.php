...
if(isset($_SESSION["uid"]) || isset($_COOKIE['user_login'])) { 
  ...
  if (isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'delete') {
      ...
      $check = mysqli_query($conn, "SELECT id FROM innovation_tbl WHERE fiscal_year = (SELECT fiscal_year FROM fiscal_year WHERE id = '$id')");
      if (mysqli_num_rows($check) > 0) {
            ...
            <div class="table-responsive">
                <table id="fiscalYearTable" class="table table-hover" style="width:100%">
                    ...
                    <tbody>
                        <?php
                        $query = "SELECT f.*, 
                                 (SELECT COUNT(*) FROM innovation_tbl WHERE fiscal_year = f.fiscal_year) as usage_count 
                                 FROM fiscal_year f 
                                 ORDER BY f.id DESC";
                        $result = mysqli_query($conn, $query);
                    ...
                    <form id="fiscalYearForm">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="fiscal_year" name="fiscal_year" 
                                   placeholder="e.g., ২০২৩-২০২৪" required>
                            <small class="text-muted">Enter fiscal year in Bengali format (e.g., ২০২৩-২০২৪)</small>
                        </div>
                    </form>
                </div>
