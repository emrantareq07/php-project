...
if (isset($_POST['save'])) {
    ...
    $max_codes = []; // track max code per division
    ...
    for ($i = 0; $i < count($designation_bn); $i++) {
        ...
        // Fetch max code from DB once per division
        if (!isset($max_codes[$division_code])) {
            $like_pattern = $division_code . '%';
            $res = $conn->query("SELECT MAX(code) AS max_code FROM all_employee WHERE code LIKE '$like_pattern'");
            $row = $res->fetch_assoc();
            if ($row && $row['max_code']) {
                $max_codes[$division_code] = (int)substr($row['max_code'], strlen($division_code));
            } else {
                $max_codes[$division_code] = 0;
            }
        }
        ...
        // Update max code for this division
...
<div class="card shadow-lg border-0">
            ...
            <div class="row mb-4">
                <div class="col-md-4">
                    ...
                    <input type="text"
                           ...
                           required>
                    <small class="text-muted">This field is for reference only. Full codes are generated automatically.</small>
                </div>
            </div>
    ...
    <div class="card-footer text-muted text-center small">
        ...
        Each row will insert "Code Count" times internally. Full code auto-generates with Division Code prefix and no duplicates.
    </div>
</div>
