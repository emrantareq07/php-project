...
function getFullFactoryName($short_name) {
    $factories = [
        ...
        'sadmin' => 'Bangladesh Chemical Industries Corporation'
    ];
...
if (!$is_admin_user) {
    ...
    if ($result_factory_name && $result_factory_name->num_rows > 0) {
        ...
        $user_factory_full = getFullFactoryName($user_factory_short);
    }
}
...
function generateSinglePrintView(data, factoryName) {
    ...
    // Use the factory name from the staff record
    const displayFactoryName = factoryName || data.factory_name || '';
