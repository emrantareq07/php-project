...
function generateCombineWorkersPrintView(monthsData) {
    ...
    monthsData.forEach(monthData => {
        ...
        summaryArray.forEach((item, index) => {
            // Check if we're starting a new grade
            if (item.grade !== currentGrade) {
                // Add grade total row if not first grade
                if (currentGrade !== '') {
                    printContent += `
                        <tr class="grade-row">
                            <td colspan="2" class="bangla-text" style="text-align: right;"><strong>${convertGradeToBangla(currentGrade)} মোট:</strong></td>
                            <td class="bangla-text"></td>
                ...
                // Reset totals for new grade
                currentGrade = item.grade;
                gradeSanctionedTotal = 0;
                gradeMaleTotal = 0;
                gradeFemaleTotal = 0;
                gradeTotal = 0;
                gradeVacantTotal = 0;
            ...
            gradeSanctionedTotal += item.sanctioned;
            gradeMaleTotal += item.male;
            gradeFemaleTotal += item.female;
            gradeTotal += item.total;
            gradeVacantTotal += item.vacant;
            
            serial++;

            // Add grade total for last item
