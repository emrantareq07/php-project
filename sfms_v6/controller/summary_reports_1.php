...
foreach ($buffers as $buf) {
    ...
    // ---- E) Daily Deliveries (To Buffer & To Dealer) ----
    if ($s = mysqli_prepare($conn, "
        SELECT COALESCE(SUM(bt.amount), 0) AS v
        FROM buffer_transaction bt
        INNER JOIN urea_allotment ua ON ua.id = bt.prod_allotment_id OR ua.id = bt.buffer_allotment_id
        WHERE bt.status = 'pending' AND DATE(bt.date) = CURDATE() AND ua.sender = ?")) {
    ...
    if ($s = mysqli_prepare($conn, "
        ...
        WHERE mt.transaction_source IN ('buffer_out','factory_out')
          AND o.buffer_name = ? AND DATE(mt.created_at) = CURDATE()")) {
        mysqli_stmt_bind_param($s, 's', $current_buffer);
        mysqli_stmt_execute($s);
        $r = mysqli_stmt_get_result($s);
    ...
    $dashStats['daily_delivery'] = $dashStats['out_ua_today'] + $dashStats['out_dealer_today'];
            ...
            <table class="table table-bordered align-middle">
                ...
                <tbody>
                    <?php if (empty($allBufferStats)): ?>
                        <tr>
                            <td colspan="12" class="text-muted">No buffer data found.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($allBufferStats as $index => $st): ?>
                            <tr>
                                ...
                                <td><?= dash($st['zone']); ?></td>
                                <td><?= h(number_format($st['capacity'], 2)); ?></td>
                                <td><?= h(number_format($st['opening_bal'], 2)); ?></td>
                                <td><?= h(number_format($st['production_today'], 2)); ?></td>

                                <td><?= h(number_format($st['daily_receive'], 2)); ?></td>
                                <td class=" fw-bold"><?= h(number_format($st['daily_delivery'], 2)); ?></td>

                                <td class="fw-bold text-success"><?= h(number_format($st['current_stock'], 2)); ?></td>
                                <td><?= h(number_format($st['pipeline_amount'], 2)); ?></td>
