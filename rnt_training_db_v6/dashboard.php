                    ...
                    <div class="row g-2">
                        ...
                        <datalist id="emp_ids">
                            ...
                            //or
                                ...
                                // Query to get emp_id from employees table
                                $sql_employees = "SELECT emp_id FROM employees";
                                $result_employees = mysqli_query($conn, $sql_employees);

                                // Query to get emp_id from bcic_staff table
                                $sql_bcic = "SELECT emp_id FROM bcic_staff";
                                $result_bcic = mysqli_query($conn, $sql_bcic);

                                // Create an array to store all unique emp_ids
                                $all_emp_ids = array();

                                // Fetch from employees table
                                while ($row = mysqli_fetch_array($result_employees)) {
                                    $all_emp_ids[$row['emp_id']] = true; // Using keys for uniqueness
                                }

                                // Fetch from bcic_staff table
                                while ($row = mysqli_fetch_array($result_bcic)) {
                                    $all_emp_ids[$row['emp_id']] = true; // Duplicates will be automatically handled
                                }
                                ...
                                //         UNION 
                                //         SELECT emp_id FROM bcic_staff
                                //         ORDER BY emp_id";
                                // $result = mysqli_query($conn, $sql);
                                // while ($row = mysqli_fetch_array($result)) {
                                //     echo "<option value='" . htmlspecialchars($row['emp_id'], ENT_QUOTES) . "'>" . 
                                //          htmlspecialchars($row['emp_id'], ENT_QUOTES) . "</option>";
                                // }
