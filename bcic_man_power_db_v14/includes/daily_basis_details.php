...
<div class="container mt-3 shadow rounded p-4">
  <h2 class="text-muted text-center text-uppercase fw-bold">দৈনিক ভিত্তিক জনবল বিস্তারিত</h2>
...
function generateCombineWorkersPrintView(monthsData) {
    ...
    let printContent = `
        ...
        <html>
        <head>
           <title>Combined Daily Basis Report - Bangladesh Chemical Industries Corporation</title>
            <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    ...
    for (var m = 0; m < monthsData.length; m++) {
        ...
        for (var i = 0; i < monthData.data.length; i++) {
            ...
            var factoryName = cleanValue(record.factory_name);

            // SAFE ARRAY CONVERSION
            var designations = record.designation
                ? record.designation.split(',')
                : ['-'];

            var grades = record.grade
                ? record.grade.split(',')
                : ['-'];

            var sanctionedPosts = record.sanctioned_post
                ? record.sanctioned_post.split(',')
                : ['0'];

            var maleCounts = record.male
                ? record.male.split(',')
                : ['0'];

            var femaleCounts = record.female
                ? record.female.split(',')
                : ['0'];

            var totalCounts = record.total
                ? record.total.split(',')
                : ['0'];
            ...
            for (var j = 0; j < maxLen; j++) {
                ...
                var designationName = cleanValue(designations[j] || '-');

                var grade = cleanValue(grades[j] || '-');

                var sanctioned = safeNumber(sanctionedPosts[j]);

                var male = safeNumber(maleCounts[j]);

                var female = safeNumber(femaleCounts[j]);

                var total = safeNumber(totalCounts[j]);
        ...
        // MONTH SECTION
            ...
            <div class="month-section">
                ...
                <h5 class="bengali-title">গ্রেড ও পদভিত্তিক সারসংক্ষেপ (দৈনিক ভিত্তিক)</h5>
        ...
        summaryArray.sort(function (a, b) {
            ...
            var gradeA =
                parseInt(
                    String(a.grade).replace(/\D/g, '')
                ) || 0;

            var gradeB =
                parseInt(
                    String(b.grade).replace(/\D/g, '')
                ) || 0;
        ...
        for (var s = 0; s < summaryArray.length; s++) {
            ...
            printContent += `
                <tr>
                    ...
                    <td>
                        ${convertGradeToBangla(item.grade)}
                    </td>

                    <td>
                        ${englishToBanglaNumber(item.sanctioned)}
                    </td>

                    <td>
                        ${englishToBanglaNumber(item.male)}
                    </td>

                    <td>
                        ${englishToBanglaNumber(item.female)}
                    </td>

                    <td>
                        ${englishToBanglaNumber(item.total)}
                    </td>

                    <td>
                        ${englishToBanglaNumber(item.vacant)}
                    </td>
        ...
        printContent += `
                <tr class="total-row">
                    ...
                    <td>
                        ${englishToBanglaNumber(totalSanctioned)}
                    </td>

                    <td>
                        ${englishToBanglaNumber(totalMale)}
                    </td>

                    <td>
                        ${englishToBanglaNumber(totalFemale)}
                    </td>

                    <td>
                        ${englishToBanglaNumber(grandTotal)}
                    </td>

                    <td>
                        ${englishToBanglaNumber(totalVacant)}
                    </td>
                ...
                <h5 class="bengali-title">
                    কারখানা/প্রতিষ্ঠান ভিত্তিক বিস্তারিত বিবরণ (দৈনিক ভিত্তিক)
                </h5>
        `;

        // FACTORY DETAILS
        for (var factory in factoryDetails) {

            var details = factoryDetails[factory];
            ...
            printContent += `
                <div style="margin-bottom:20px;">
                    ...
                    ">
                        ...
                        ${convertToBanglaFactory(factory)}
                    </h6>
            ...
            for (var d = 0; d < details.length; d++) {
                ...
                printContent += `
                    <tr>
                        ...
                        <td>
                            ${convertGradeToBangla(detail.grade)}
                        </td>

                        <td>
                            ${englishToBanglaNumber(detail.sanctioned)}
                        </td>

                        <td>
                            ${englishToBanglaNumber(detail.male)}
                        </td>

                        <td>
                            ${englishToBanglaNumber(detail.female)}
                        </td>

                        <td>
                            ${englishToBanglaNumber(detail.total)}
                        </td>

                        <td>
                            ${englishToBanglaNumber(detail.vacant)}
                        </td>
            ...
            printContent += `
                    <tr class="factory-total-row">
                        ...
                        <td>
                            ${englishToBanglaNumber(fs)}
                        </td>

                        <td>
                            ${englishToBanglaNumber(fm)}
                        </td>

                        <td>
                            ${englishToBanglaNumber(ff)}
                        </td>

                        <td>
                            ${englishToBanglaNumber(ft)}
                        </td>

                        <td>
                            ${englishToBanglaNumber(fv)}
                        </td>
        ...
        printContent += `
            <div class="text-center mt-3">
                ...
                <strong>
                    <small>
                        সিস্টেম জেনারেটেড ডকুমেন্ট। স্বাক্ষরের প্রয়োজন নাই।
                    </small>
                </strong>
    ...
    var printWindow = window.open(
        '',
        '_blank',
        'width=1200,height=800,scrollbars=1'
    );
...
function generateSingleWorkerPrintView(data) {
    ...
    if (validEntries.length > 0) {
        ...
        for (var e = 0; e < validEntries.length; e++) {
            ...
            var isPlaceholder = entry.designation === 'পদ উল্লেখ নেই';
