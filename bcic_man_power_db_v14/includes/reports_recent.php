...
function renderTable(data, type) {
    ...
    if (type === 'officer') {
        ...
        return renderStaffTable(data); // Fallback to staff table
    }
}
    ...
    return `<table class="table table-bordered table-striped report-table">
        <thead>
            <tr>
                ...
                <th rowspan="2" style="width: 25%;">বিভাগের নাম</th>
...
function triggerIsolatedPrint() {
    ...
    let printContent = `
        ...
        <html>
        <head>
            <title>Employee Data Report - ${printData.date}</title>
            <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    ...
    // Add combined records info if multiple records
    if (currentData.totalCount > 1) {
        printContent += `
    ...
    printContent += `
                <div class="print-header text-center">
                    ...
                    <h5 class="mb-1">কারখানা/প্রতিষ্ঠান/প্রকল্পের নাম : ${printData.factory_name}</h5>                  
                    <h6 class="mb-0">${titleText} : (${convertDateToBangla(printData.date)} তারিখে)</h6>
                </div>
    `;
    ...
    printContent += `
                ...
                <div class="text-center mt-2">
                    <small class="text-muted bangla-number">প্রতিবেদন তৈরির তারিখ: ${convertDateToBangla(currentDate)}</small>
                </div>
            </div>
        </body>
        </html>
