...
<div class="container mt-3 shadow rounded p-4">
  <h2 class="text-muted text-center text-uppercase fw-bold">সকল কর্মকর্তা জনবল বিস্তারিত</h2>
...
function generateFactoryCombinePrintView(data) {
    ...
    printContent += `
                <div class="no-print text-center mt-1">
...
function generateCombinePrintView(monthsData) {
    ...
    let printContent = `
        ...
        <head>
            ...
            <style>
                ...
                /* =====================================================
                   REMOVED: page-break-after: always from .month-section
                   This was the ROOT CAUSE of the page break issue.
                   ===================================================== */
                .month-section {
                    margin-bottom: 10px;
                    /* NO page-break-after here */
                }
                ...
                .month-title h3 { margin-bottom: 3px; }
                .month-title p  { margin-bottom: 0; }
                ...
                /* ======================================================
                   PRINT STYLES — Single Page Fix
                   ====================================================== */
                @media print {

                    @page {
                        size: Letter landscape;   /* wider page gives more room */
                        margin: 10mm;
                    }

                    /* Scale everything down to fit one page.
                       Adjust the zoom value (0.7–1.0) as needed. */
                    body {
                        margin: 0 !important;
                        font-size: 16px !important;
                        zoom: 0.75;           /* <-- PRIMARY single-page fix */
                    }

                    /* Allow the whole document to flow naturally — NO forced breaks */
                    .month-section {
                    ...
                    /* Header should never break away from content */
                    .print-header {
                        page-break-after: avoid !important;
                        break-after: avoid !important;
                    }

                    /* Only avoid breaks on small containers, NOT on table/tbody/tr
                       (those caused the 2nd-page overflow) */
                    .month-title,
                    .combined-info {
                        page-break-inside: avoid !important;
                        break-inside: avoid !important;
                    ...
                    /* thead repeats on every page if content does overflow */
                    thead {
                        display: table-header-group !important;
                    }

                    /* Remove orphan/widow constraints that fight single-page layout */
                    p, h1, h2, h3, h4, h5, h6 {
                        orphans: 2;
                        widows: 2;
                    }
                    ...
                    /* Tighten table cells for compact layout */
                    table td, table th {
    ...
    monthsData.forEach(monthData => {
        ...
        monthData.data.forEach(record => {
            grades.forEach(grade => {
                if (record[grade + '_m']) {
                    record[grade + '_m'].split(',').forEach((value, index) => {
                        combinedData[grade + '_m'][index] += parseInt(value || 0);
                    });
                }
                if (record[grade + '_f']) {
                    record[grade + '_f'].split(',').forEach((value, index) => {
                        combinedData[grade + '_f'][index] += parseInt(value || 0);
                    });
                }
            });
        ...
        const factoryCount = englishToBanglaNumber(
            Object.keys(monthData.data.reduce((acc, record) => {
        ...
        <div class="month-section">
            ...
            <div class="month-title ">
                ...
                <p class="bangla-text">
                    মোট কারখানা: ${factoryCount} | মোট রেকর্ড: ${englishToBanglaNumber(monthData.records_count)}
                </p>
            </div>
        ...
        // <!-- Print only -->
        //     <div class="month-title print-only">
        //         <h5 class="bangla-text">
        //             মাস: ${convertToBanglaMonth(monthData.month_name)} &nbsp;|&nbsp;
        //             মোট কারখানা: ${factoryCount} &nbsp;|&nbsp;
        //             মোট রেকর্ড: ${englishToBanglaNumber(monthData.records_count)}
        //         </h5>
        //     </div>
    ...
    const printWindow = window.open('', '_blank', 'width=1400,height=900,scrollbars=1');
    printWindow.document.write(printContent);
    printWindow.document.close();
    printWindow.focus();
}
