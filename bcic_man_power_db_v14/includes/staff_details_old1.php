...
function getFullFactoryName($short_name) {
    $factories = [
        'SFCL' => 'Dhaka Fertilizer Limited',
        'UFFL' => 'Urea Fertilizer Factory Limited', 
        'CFCL' => 'Chittagong Fertilizer Company Limited',
        'TSPCL' => 'TSP Complex Limited',
        'KAFCO' => 'Karnaphuli Fertilizer Company Limited',
        'ZFL' => 'Zia Fertilizer Limited',
        'BCIC' => 'Bangladesh Chemical Industries Corporation',
        'admin' => 'Bangladesh Chemical Industries Corporation'
    ];
    
    // If short_name is already a full name or not in mapping, return as is
    return isset($factories[$short_name]) ? $factories[$short_name] : $short_name;
}
...
// Get factory_name from session (from users table)
$session_factory_short = $_SESSION['factory_name'] ?? '';
$session_factory_full = getFullFactoryName($session_factory_short);
    ...
    <tbody>
      <?php if ($result && $result->num_rows > 0): ?>
        ...
        while ($row = $result->fetch_assoc()): 
          ...
          $factory_full = getFullFactoryName($factory_short);
        ...
        <?php endif; ?>
          ...
          <tr>
            ...
            <td><?php echo htmlspecialchars($factory_full); ?></td>
            <td>
              <?php 
            ...
            <td class="no-print">
                ...
                <button class="btn btn-success btn-sm print-btn" 
                        onclick="printSingleRecord(<?php echo $row['id']; ?>, '<?php echo htmlspecialchars($session_factory_full); ?>')"
...
// Print single record with minimized margins and session factory name
function printSingleRecord(id, sessionFactoryFull, event) {
    ...
    $.ajax({
        ...
        success: function(response) {
            try {
                ...
                if (response.success) {
                    // Pass the session factory name to the print view
                    generateSinglePrintView(response.data, sessionFactoryFull);
                } else {
...
$(document).ready(function() {
    ...
    $(document).on('click', '.print-btn', function(e) {
        ...
        const sessionFactoryFull = $(this).attr('data-session-factory') || '';
        printSingleRecord(id, sessionFactoryFull, e);
    });
});
...
// ✅ Single print view with minimized margins and session factory name
function generateSinglePrintView(data, sessionFactoryFull) {
    ...
    // Use session factory name if available, otherwise use data factory name
    const displayFactoryName = sessionFactoryFull || data.factory_name || '';
    ...
    <head>
        ...
        <style>
            @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+Bengali:wght@400;700&display=swap');
            /* Single record print - minimized margins */
            * {
                margin: 0;
                padding: 0;
    ...
    <body>
        <div class="container">
            <div class="print-header">
                ...
                <h4 class="bangla-text">বিসিআইসি ভবন, ৩০-৩১, দিলকুশা বা/এ, ঢাকা-১০০০</h4>
                <h4 class="bangla-text">কর্মকর্তা/কর্মচারী বিস্তারিত</h4>
                <h5 class="bangla-text">কারখানা নাম : ${displayFactoryName}</h5>
...
// Update print buttons with session factory name
$(document).ready(function() {
    $('.print-btn').each(function() {
        $(this).attr('data-session-factory', '<?php echo htmlspecialchars($session_factory_full); ?>');
    });
});
</script>
