    ...
    <div class="row align-items-center">
      <div class="col-md-6">
        <h4 class="mb-1 text-primary">
          <i class="fas fa-users me-2"></i>BCIC Officer's Info. Data Entry (Only In-Service)
        </h4>
...
$(document).on('click', '.print-btn', function() {
    ...
    $printBtn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin me-1"></i>Loading...');
     ...
     $.ajax({
        ...
        data: { 
            id: id,
            mode: 'single' // Use single mode for printing
        },
        success: function(response) {
            try {
                if (typeof response !== 'object') response = JSON.parse(response);
...
// FIXED: Function to generate print view with correct totals matching entry totals
function generatePrintView(data) {
    const sections = <?php echo json_encode($sections); ?>;
    const grades = <?php echo json_encode($grades); ?>;
    
    // Calculate grand totals properly - MUST MATCH calculateAllTotals() logic
    let grandMaleTotal = 0;
    let grandFemaleTotal = 0;
    let grandTotal = 0;
    
    // First calculate all section totals
    const sectionTotals = [];
    for (let i = 0; i < sections.length; i++) {
        ...
        for (let g = 0; g < grades.length; g++) {
            ...
            const maleKey = grade + '_m';
            const femaleKey = grade + '_f';
            
            // Get male values for this section
            let maleVal = 0;
            if (data[maleKey] && data[maleKey] !== '') {
                const maleValues = data[maleKey].split(',');
                if (i < maleValues.length && maleValues[i] !== '') {
                    maleVal = parseInt(maleValues[i]);
                    if (isNaN(maleVal)) maleVal = 0;
                }
            }
            
            // Get female values for this section
            let femaleVal = 0;
            if (data[femaleKey] && data[femaleKey] !== '') {
                const femaleValues = data[femaleKey].split(',');
                if (i < femaleValues.length && femaleValues[i] !== '') {
                    femaleVal = parseInt(femaleValues[i]);
                    if (isNaN(femaleVal)) femaleVal = 0;
                }
            }
            
            sectionMale += maleVal;
            sectionFemale += femaleVal;
        }
        
        sectionTotals[i] = { male: sectionMale, female: sectionFemale, total: sectionMale + sectionFemale };
        grandMaleTotal += sectionMale;
        grandFemaleTotal += sectionFemale;
        grandTotal += (sectionMale + sectionFemale);
    }
    ...
    for (let g = 0; g < grades.length; g++) {
        ...
        for (let i = 0; i < sections.length; i++) {
            if (data[maleKey] && data[maleKey] !== '') {
                const maleValues = data[maleKey].split(',');
                if (i < maleValues.length && maleValues[i] !== '') {
                    let val = parseInt(maleValues[i]);
                    if (!isNaN(val)) gradeMale += val;
                }
            }
            if (data[femaleKey] && data[femaleKey] !== '') {
                const femaleValues = data[femaleKey].split(',');
                if (i < femaleValues.length && femaleValues[i] !== '') {
                    let val = parseInt(femaleValues[i]);
                    if (!isNaN(val)) gradeFemale += val;
                }
            }
        }
        gradeTotals[grade] = { male: gradeMale, female: gradeFemale, total: gradeMale + gradeFemale };
    }
    ...
    // Add data rows with Bangla numbers - FIXED to use pre-calculated section totals
    let serialNumber = 1;
    ...
    sections.forEach((section, index) => {
        ...
        grades.forEach(grade => {
            ...
            if (data[maleKey] && data[maleKey] !== '') {
                const maleValues = data[maleKey].split(',');
                if (index < maleValues.length && maleValues[index] !== '') {
                    maleVal = parseInt(maleValues[index]);
                    if (isNaN(maleVal)) maleVal = 0;
                }
            }
            ...
            if (data[femaleKey] && data[femaleKey] !== '') {
                const femaleValues = data[femaleKey].split(',');
                if (index < femaleValues.length && femaleValues[index] !== '') {
                    femaleVal = parseInt(femaleValues[index]);
                    if (isNaN(femaleVal)) femaleVal = 0;
                }
            }
            
            const gradeTotal = maleVal + femaleVal;
            
            printContent += `
                    <td class="male-col bangla-number">${englishToBanglaNumber(maleVal)}</span>
                    <td class="female-col bangla-number">${englishToBanglaNumber(femaleVal)}</span>
                    <td class="grade-total bangla-number">${englishToBanglaNumber(gradeTotal)}</span>
            `;
        });
        
        // Use pre-calculated section totals
        printContent += `
                    <td class="male-col section-total bangla-number">${englishToBanglaNumber(sectionTotals[index].male)}</span>
                    <td class="female-col section-total bangla-number">${englishToBanglaNumber(sectionTotals[index].female)}</span>
                    <td class="grade-total section-total bangla-number">${englishToBanglaNumber(sectionTotals[index].total)}</span>
                </tr>
        `;
    ...
    // Display grade-wise grand totals
    grades.forEach(grade => {
        const gradeData = gradeTotals[grade];
        printContent += `
            <td class="male-col grand-total bangla-number"><strong>${englishToBanglaNumber(gradeData.male)}</strong></span>
            <td class="female-col grand-total bangla-number"><strong>${englishToBanglaNumber(gradeData.female)}</strong></span>
            <td class="grade-total grand-total bangla-number"><strong>${englishToBanglaNumber(gradeData.total)}</strong></span>
        `;
    });
    ...
    printContent += `
        ...
        <div class="row mt-0">
            <div class="col-md-12 text-center">
                <div style="border-top: ০px solid #000; width: auto; margin: 0 auto; padding-top: 0px;">
