...
$total_query = "SELECT COUNT(*) AS total FROM $table_name WHERE immediate_sender_office=''  order by id DESC";
		...
		<div class="table-title">
			<div class="row">
				...
				<div class="col-sm-9 text-end">
					...
					if($user_type=='sadmin'){   
						?>
						<h4><a href="backend/manage_user.php?username=<?=$_SESSION['username']?>" class="btn btn-warning"><i class="fa fa-edit" style="font-size:15px;color:black"></i> Manage User </a>
						</h4>
							<h4>
						<a href="backend/manage_user.php?username=<?=$_SESSION['username']?>" class="btn btn-warning"><i class="fa fa-download" style="font-size:15px;color:black"></i> Download DB </a>
						</h4>
						<?php
						 }
						?>
                    ...
                    <form id="downloadForm" action="download_database.php" method="post" class="d-inline-block">
                        <button class="btn btn-warning" type="submit" name="submit">
                            <i class="fa fa-download" style="font-size:16px"></i> Download DB
                        </button>
                    </form>
                <?php } ?>	
                    <button type="button" class="btn btn-danger" id="print_current_date"><i class="fa fa-print" style="font-size:16px"></i> Print</button>
					<a href="logout.php" class="btn btn-danger" ><i class="fa fa-sign-out" style="font-size:16px"></i><span> Logout</span></a>	
...
</div><span class="text-warning fw-bold"><small>Logged As a : [--<?php echo $user_type; ?>--]</small> &nbsp;&nbsp;<small class="text-primary fw-bold">Total Records : <?= englishToBanglaNumber($total_pages) ?> 
        </small></span>
...
<div id="printableArea_current_date">
    ...
    // $result = mysqli_query($conn, "SELECT * FROM $table_name where immediate_sender_office='' ORDER BY id DESC");

    $result = mysqli_query($conn, "SELECT * FROM $table_name WHERE immediate_sender_office='' ORDER BY CAST(id AS UNSIGNED) DESC");
    //$i = $total_rows;
    $i = 1;
        ...
        while ($row1 = mysqli_fetch_array($result)) {
            ...
            // while ($row = mysqli_fetch_array($result_for_chair)) {
                ...
                <tr id="<?php echo $row1["id"]; ?>" class="text-center">
                    ...
                    <td class="text-justify"><?php echo $row1["subject"]; ?></td>
                    ...
                    <a href="#viewEmployeeModal" class="edit" data-toggle="modal" style="text-decoration: none">
                        ...
                        <a href="edit_show_all.php?id=<?= $row1['id'] ?>" class="">
...
<script>
   document.getElementById('searchInput').addEventListener('keyup', function () {
    ...
    xhr.open("POST", "search_ref_no_show_all.php", true);
