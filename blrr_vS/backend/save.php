...
if(count($_POST)>0){
    ...
    // if($_POST['type']==1){
        ...
        $medium=$_POST['medium'];
        $destination_drop = $_POST['selected_destinations'];
    ...
    if ( $recipient && $d_number  && $send_date && $sender && $subject && $medium ) {
     $sql = "INSERT INTO `$table_name`(`entry_date`, `recipient`, `d_number`, `attention`, `ref_number`, `send_date`, `sender`, `div_dept_office`, `subject`,`destination_drop`,`distribution_date`, `chairman_note`, `comments`, `medium`, `status`) 
                VALUES ('$entry_date','$recipient','$d_number','$attention','$ref_number','$send_date','$sender','$div_dept_office','$subject','$destination_drop','$distribution_date','$chairman_note','$comments','$medium','$status')";
