...
if (!empty($imageFile) && file_exists($imagePath)) {
    ...
    $userImage = "admin/student_images/default.png"; // fallback image
}
?>
...
<div class="container py-5">
    <div class="row mb-4">
        <?php
            ...
            $studentId = $_SESSION['stdid']; // Assuming you store this in session
    ...
    <div class="row my-4">
        ...
        <div class="col-md-6 position-relative">
            ...
            <datalist id="bookList">
                <?php
                $sql = "SELECT DISTINCT b.edition, b.book_name, c.CategoryName, a.AuthorName, b.author_id, b.category_id
                        FROM tblbooks b
                        JOIN tblcategory c ON b.category_id = c.id
                        JOIN tblauthors a ON b.author_id = a.id";
                $result = mysqli_query($conn, $sql);
                while ($row = mysqli_fetch_array($result)) {
                    echo "<option value='" . htmlspecialchars("{$row['CategoryName']} - {$row['book_name']} - {$row['AuthorName']} ({$row['edition']})", ENT_QUOTES) . "' 
...
function fetchAccessionNumbers(bookName, authorId, categoryId) {
    ...
    .then(data => {
        ...
        output.value = data.success ? data.accession_numbers.join('') : 'Book Issued By Someone. Plz Try Again!!!';
    })
    .catch(err => {
        console.error(err);
