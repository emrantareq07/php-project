...
if(isset($_POST['save_org'])){            
    ...
    $establish_year = mysqli_real_escape_string($conn, $_POST['establish_year']);   

    $query = "INSERT INTO org_tbl (org_name,address,establish_year) VALUES ('$org_name','$address','$establish_year')";

    $query_run = mysqli_query($conn, $query);
    if($query_run) {
        $_SESSION['message'] = "<span class='text-success'><b>Org. Created Successfully</b></span>";
        header("Location: org_details.php");
        exit(0);
    }
    else{
        $_SESSION['message'] = "<span class='text-danger'><b>Org. Not Created</b></span>";
        header("Location: org_details.php");
        exit(0);
    }
}
...
if(isset($_POST['update_org'])){  
    ...
    $query = "UPDATE org_tbl SET  org_name='$org_name', address='$address', establish_year='$establish_year' WHERE id='$id' ";    

    $query_run = mysqli_query($conn, $query);
    if($query_run) {
        $_SESSION['message'] = "<span class='text-success'><b>Org. Updated Successfully</b></span>";
        header("Location: org_details.php");
        exit(0);
    }
    else{
        $_SESSION['message'] = "<span class='text-danger'><b>Org. Not Updated</b></span>";
        header("Location: org_details.php");
        exit(0);
    }
}
...
if(isset($_POST['delete_org'])){
    $id = mysqli_real_escape_string($conn, $_POST['delete_org']);
    ...
    $query_run = mysqli_query($conn, $query);
     ...
     $query_run = mysqli_query($conn, $query);
    if($query_run) {
        $_SESSION['message'] = "<span class='text-success'><b>Org. Deleted Successfully</b></span>";
        header("Location: org_details.php");
        exit(0);
    }
