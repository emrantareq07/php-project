...
$dump = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=land_db', 'root', '');
 $dump->start($outputFile);


$dump1 = new Ifsnop\Mysqldump\Mysqldump('mysql:host=localhost;dbname=land_db', 'root', '');
$f=date('d-m-Y');
$dump1->start("db/land_db_$f.sql");
...
$mail->IsHTML(true);
$mail->Subject="BCIC Land Database Backup ".$f;
$mail->Body="Database Backup";
$mail->AddAttachment("db/land_db_$f.sql");
...
if (isset($_POST['submit'])) {
    $backupFilename = 'db/land_db_' . date('d-m-Y') . '.sql';
