            ...
            <div class="card-body">
                <form method="POST" action="searching_info.php" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-3">
                        <!-- <input type="emp_id" class="form-control" id="emp_id" placeholder="Enter EMP ID" name="emp_id" required> -->
                        <label for="emp_id">EMP ID</label>
                         <input list="StudentIdList" type="text" class="form-control emp_id" name="emp_id" placeholder="Enter EMP ID"  oninput="fetchStudentDetails(this)">
                    <datalist id="StudentIdList">
                        <?php
                            $sql = "SELECT emp_id FROM employees";
                            $result = mysqli_query($conn, $sql);
                            while ($row = mysqli_fetch_array($result)) {
                                echo "<option value='" . htmlspecialchars($row['emp_id'], ENT_QUOTES, 'UTF-8') . "'>";
                            }
                        ?>
                    </datalist>
                    </div>
